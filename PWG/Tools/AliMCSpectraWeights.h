...
private:
  ...
  std::map<AliMCSpectraWeights::SysFlag, TH3F*> fHistMCWeightsSys;  /*!< Histograms for systematic variations 
                                                                         of weight factors */
  TRandom3 frndGen;
  ...
  TH3F* fHistRestCorrVal;       /*!< Histogram for correction factors of rem. particle species (1-f_rem) */
  ...
  TH3F* fHistMCWeightsSysUp;    /*!< Histograms for average systematic of weight factors  moved up*/
  TH3F* fHistMCWeightsSysDown;  /*!< Histograms for average systematic of weight factors  moved down*/
  ...
  bool fUseMBFractions;         /*!< switch to use MB fractions instead of mult. dependent ones*/
  bool fDoSystematics;
...
public:
  ...
  AliMCSpectraWeights (std::string const &collisionSystem, std::string const &stName,
  ...
  float const GetMCSpectraWeight (Int_t mcGenParticle, AliMCEvent* mcEvent); /*!< old; should not be used */

  float const GetMCSpectraWeight (Int_t mcGenParticle, Int_t SysCase=0); /*!< way to go; SysCase = 0 --> nominal;
    -1 --> sys. moved down; +1 --> sys. moved up */

  float const GetMCSpectraWeight (TParticle* mcGenParticle, Int_t SysCase=0); /*!< way to go; SysCase = 0 -->
    nominal; -1 --> sys. moved down; +1 --> sys. moved up */

  float const GetMCSpectraWeightNominal (Int_t mcGenParticle);  /*!< main function to use. Will deliver
    correct weights to re-weight the abundances of different particle species */
  float const GetMCSpectraWeightNominal (TParticle* mcGenParticle);  /*!< main function to use. Will deliver
    correct weights to re-weight the abundances of different particle species */

  float const GetMCSpectraWeightSystematics (Int_t mcGenParticle, Int_t SysCase = 1);
  float const GetMCSpectraWeightSystematics (TParticle* mcGenParticle, Int_t SysCase = 1);

  int   const IdentifySecondaryType         (Int_t partLabel);
  float const GetWeightForSecondaryParticle (Int_t partLabel, Int_t SysCase=0);

  void FillMCSpectra(AliMCEvent* mcEvent);  /*!< function to fill internal mc spectra for calculation
                                                  of weight factors */
  void StartNewEvent();
  ...
  TH3F* const GetHistRestCorrVal                     () const {return fHistRestCorrVal;}
  ...
  ClassDef(AliMCSpectraWeights, 2);
};
