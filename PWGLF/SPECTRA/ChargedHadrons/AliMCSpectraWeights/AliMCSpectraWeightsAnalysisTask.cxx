...
AliMCSpectraWeightsAnalysisTask::AliMCSpectraWeightsAnalysisTask(const char* name) :
...
void AliMCSpectraWeightsAnalysisTask::UserExec(Option_t* option) {
  if(!fIsMC) {
    printf("AliMCSpectraWeightsAnalysisTask:: WARNING task only works for MC!!\n");
    return;
  }
...
AliMCSpectraWeightsAnalysisTask* AliMCSpectraWeightsAnalysisTask::AddTaskWeights(const char* collisionSystem,
  const char* previousTrain, const char* name, const char* outfile)
{
  ...
  AliMCSpectraWeightsAnalysisTask* task = new AliMCSpectraWeightsAnalysisTask(name);
  task->SetUseMC((AliAnalysisManager::GetAnalysisManager()->GetMCtruthEventHandler() != 0x0));
  ...
  mgr->ConnectOutput(task, 1, mgr->CreateContainer(name, TList::Class(),AliAnalysisManager::kOutputContainer,
                                                   fileName.Data()));
