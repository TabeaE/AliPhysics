...
class AliReducedTrackCut : public AliReducedVarCut {
 ...
 public:
  ...
  void SetITShitRequest(UChar_t hitMap, Bool_t useAND=kFALSE) {fCutOnITShitMap        = hitMap;
                                                               fUseANDonITShitMap     = useAND; 
                                                               fRequestCutOnITShitMap = kTRUE;}
  void SetRequestITSanyCut(Int_t layer) {for(Int_t i=0;i<=layer;++i) fCutOnITShitMap |= (1<<i);
                                         fUseANDonITShitMap = kFALSE; fRequestCutOnITShitMap = kTRUE;}
  void SetRequestSPDfirst () {fCutOnITShitMap |= (1<<0); fUseANDonITShitMap = kFALSE;
                              fRequestCutOnITShitMap = kTRUE;}
  void SetRequestSPDany   () {fCutOnITShitMap |= (1<<0);   fCutOnITShitMap |= (1<<1);
                              fUseANDonITShitMap = kFALSE; fRequestCutOnITShitMap = kTRUE;}
  void SetRequestSPDboth  () {fCutOnITShitMap |= (1<<0);   fCutOnITShitMap |= (1<<1);
                              fUseANDonITShitMap = kTRUE;  fRequestCutOnITShitMap = kTRUE;}
  void SetITSsharedClsRejectMap (UChar_t sharedMap, Bool_t useAND=kFALSE) {
    fCutOnITSsharedClsMap        = sharedMap;
    fUseANDonITSsharedClsMap     = useAND;
    fRequestCutOnITSsharedClsMap = kTRUE;}
  void SetRejectSharedClsOnLayer (Int_t layer) {
    if(layer<0 || layer>5) return;
    fCutOnITSsharedClsMap       |= (1<<layer);
    fUseANDonITSsharedClsMap     = kFALSE;
    fRequestCutOnITSsharedClsMap = kTRUE;}
  void SetRejectSharedClsSPDany  () {fCutOnITSsharedClsMap       |= (1<<0);
                                     fCutOnITSsharedClsMap       |= (1<<1);
                                     fUseANDonITSsharedClsMap     = kFALSE;
                                     fRequestCutOnITSsharedClsMap = kTRUE;}
  void SetRejectSharedClsSPDboth () {fCutOnITSsharedClsMap       |= (1<<0);
                                     fCutOnITSsharedClsMap       |= (1<<1);
                                     fUseANDonITSsharedClsMap     = kTRUE;
                                     fRequestCutOnITSsharedClsMap = kTRUE;}
  ...
  void SetTrackFilterMap (UInt_t map, Bool_t useAND=kTRUE) {fCutOnTrackFilterMap    = map;
                                                            fUseANDonTrackFilterMap = useAND;}
  ...
  void SetTrackQualityFilterMap(UInt_t map, UInt_t exclusionMap=0, Bool_t useAND=kTRUE) {
     fCutOnTrackQualityMap        = map;
     fCutOnTrackQualityMapExclude = exclusionMap;
     fUseANDonTrackQualityMap     = useAND;
  }
 ...
 protected: 
  ...
  Bool_t  fUseANDonMCFilterMap;  // If false, at least one of the enabled positions in the cut map 
                                 //  should be on in the track MC signals map.
  ...
  Bool_t  fUseANDonITShitMap;    // If false, at least one of the enabled positions in the cut map 
                                 //  should have corresponding clusters.
                                 ...
                                 //  e.g. 110000 and FALSE: equivalent to SPDany (at least one hit in
                                 //                         the first two layers)
                                 //  e.g. 110000 and TRUE:  equivalent to requesting both SPD layers to 
                                 //                         have a hit
  Bool_t  fRequestCutOnITShitMap;    // If true, apply the cut above
  UChar_t fCutOnITSsharedClsMap;     // Map encoding various requests on shared cluster configurations 
                                     //  to reject.
  Bool_t  fUseANDonITSsharedClsMap;  // If false, at least one of the enabled positions in the map 
                                     //  should have corresponding shared clusters to reject.
                                     //  If true, all the enabled bits should have shared clusters to 
                                     //  reject.
                                     //  The bits left to zero will not be used when evaluating the cut
                                     //  e.g. 100000 and TRUE:  reject tracks with a shared cls in the 
                                     //                         first layer
                                     //  e.g. 110000 and TRUE:  reject tracks with shared cls in both of 
                                     //                         the first 2 layers
