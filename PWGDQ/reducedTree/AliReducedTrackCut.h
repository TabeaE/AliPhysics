...
class AliReducedTrackCut : public AliReducedVarCut {
 ...
 public:
  ...
  void SetITShitRequest(UChar_t hitMap, Bool_t useAND=kFALSE) {fCutOnITShitMap = hitMap;
    fUseANDonITShitMap = useAND; fRequestCutOnITShitMap = kTRUE;}
  ...
  void SetRequestSPDany() {fCutOnITShitMap |= (1<<0); fCutOnITShitMap |= (1<<1); fUseANDonITShitMap = kFALSE;
    fRequestCutOnITShitMap = kTRUE;}
  void SetRequestSPDboth() {fCutOnITShitMap |= (1<<0); fCutOnITShitMap |= (1<<1); fUseANDonITShitMap = kTRUE;
    fRequestCutOnITShitMap = kTRUE;}
  void SetITSsharedClsRejectMap(UChar_t sharedMap, Bool_t useAND=kFALSE) {fCutOnITSsharedClsMap = sharedMap;
    fUseANDonITSsharedClsMap = useAND; fRequestCutOnITSsharedClsMap = kTRUE;}
  void SetRejectSharedClsOnLayer(Int_t layer) {if(layer<0 || layer>5) return;
    fCutOnITSsharedClsMap |= (1<<layer); fUseANDonITSsharedClsMap = kFALSE;
    fRequestCutOnITSsharedClsMap = kTRUE;}
  void SetRejectSharedClsSPDany() {fCutOnITSsharedClsMap |= (1<<0); fCutOnITSsharedClsMap |= (1<<1);
    fUseANDonITSsharedClsMap = kFALSE; fRequestCutOnITSsharedClsMap = kTRUE;}
  void SetRejectSharedClsSPDboth() {fCutOnITSsharedClsMap |= (1<<0); fCutOnITSsharedClsMap |= (1<<1);
    fUseANDonITSsharedClsMap = kTRUE; fRequestCutOnITSsharedClsMap = kTRUE;}
 ...
 protected: 
  ...
  Bool_t  fUseANDonITSsharedClsMap;  // if false, at least one of the enabled positions in the map should have
                                     // corresponding shared clusters to reject
