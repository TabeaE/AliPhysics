...
class AliReducedBaseTrack : public TObject {
  ...
  public:
    ...
    Float_t GetBayesProb(Int_t specie)       const {return (fQualityFlags&(ULong_t(1)<<(15+specie)) ?
                                                           ...
                                                           (fQualityFlags&(ULong_t(1)<<19) ? 0.7 : 0.5))):0.0);}
    
    UInt_t  GetMCFlags()                     const {return fMCFlags;}
    Bool_t  TestMCFlag(UShort_t iflag)       const {return ((iflag<(8*sizeof(UInt_t))) ?
                                                    fMCFlags & (UInt_t(1)<<iflag) : kFALSE);}
    ...
    Bool_t SetQualityFlag(UShort_t iflag)   {if(iflag>=8*sizeof(ULong_t)) return kFALSE;
                                             fQualityFlags|=(ULong_t(1)<<iflag); return kTRUE;}
