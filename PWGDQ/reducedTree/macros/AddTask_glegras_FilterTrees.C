...
void DefineHistograms(AliReducedAnalysisFilterTrees* task, Bool_t ispp/*=kTRUE*/, TString prod /*="LHC10h"*/) {
  ...
  for(Int_t iclass=0; iclass<arr->GetEntries(); ++iclass) {
    ...
    if(classStr.Contains("CorrelMult")) { //Correlations between different multiplicity estimators
      man->AddHistClass(classStr.Data());
      cout << classStr.Data() << endl;
      man->AddHistogram(classStr.Data(),"NGlobalTracksNoDCA_NGlobalTracks","Number of global tracks without DCA requirement vs with DCA",kFALSE, nMultBins, minMult, maxMult, AliReducedVarManager::kNGlobalTracks, nMultBins, minMult, maxMult, AliReducedVarManager::kNGlobalTracks + 1);
      continue;
    }
