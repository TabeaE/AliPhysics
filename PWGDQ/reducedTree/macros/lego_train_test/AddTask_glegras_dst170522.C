...
AliAnalysisTask *AddTask_glegras_dst(Int_t reducedEventType=-1, Bool_t writeTree=kTRUE, TString  prod="") {
    ...
    task->AddTrackFilter(AssociatedHadronFilterNoDCA(isAOD, 1),  kTRUE);   // base track info, fQualityFlag bit 53 - unid. hadron, ITS + TPC refit
    task->AddTrackFilter(AssociatedHadronFilterNoDCA(isAOD, 2),  kTRUE);   // base track info, fQualityFlag bit 54 - unid. hadron, ITS + TPC refit, SPD any
    ...
    task->AddTrackFilter(AssociatedHadronFilterNoDCA(isAOD, 4),  kTRUE);   // base track info, fQualityFlag bit 55 - unid. hadron, ITS + TPC refit, no kinks
    task->AddTrackFilter(AssociatedHadronFilterNoDCA(isAOD, 5),  kTRUE);   // base track info, fQualityFlag bit 56 - unid. hadron, ITS + TPC refit, SPD any, no kinks
...
AliAnalysisCuts* TPCChi2Cut(Bool_t isAOD, int spec) {
  ...
  AliDielectronCutGroup*  tpcChi2Cut = new AliDielectronCutGroup("TPCChi2Cut", "TPCChi2Cut", AliDielectronCutGroup::kCompAND);
  AliDielectronVarCuts*   trackCuts = new AliDielectronVarCuts("trackCuts", "track cuts");
  trackCuts->AddCut(AliDielectronVarManager::kImpactParXY,              -1.0, 1.0);
  trackCuts->AddCut(AliDielectronVarManager::kImpactParZ,               -3.0, 3.0);
  trackCuts->AddCut(AliDielectronVarManager::kEta,                      -0.9, 0.9);
...
AliAnalysisCuts* QualityCut(Bool_t isAOD, Int_t spec) {
  ...
  AliDielectronCutGroup*  qualityCut = new AliDielectronCutGroup("qualityCut", "qualityCut", AliDielectronCutGroup::kCompAND);
