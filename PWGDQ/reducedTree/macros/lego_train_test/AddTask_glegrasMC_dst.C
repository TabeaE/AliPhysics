...
AliAnalysisCuts* TPCChi2Cut(Bool_t isAOD);
...
AliAnalysisTask *AddTask_glegras_dst(Int_t reducedEventType=-1, Bool_t writeTree=kTRUE, TString  prod="") {
    ...
    //task->SetWriteUnbiasedEvents(0.02); NO need to write unbiased events in MC
    ...
    task->AddTrackFilter(JPsiElectronFilterLowMom(isAOD),   kFALSE);  // full track info, fQualityFlag bit 32 - all momentum
    //task->AddTrackFilter(JPsiElectronFilterHighMom(isAOD),  kFALSE);  // full track info, fQualityFlag bit 33 - high momentum - not used now 
    ...
    task->AddTrackFilter(TPCChi2Cut(isAOD),                 kTRUE);   // base track info, fQualityFlag bit 47 - unid. hadrons, TPC refit + TPC chi2 cut

if(!isAOD) {
    task->AddTrackFilter(QualityCut(isAOD, 0),                  kTRUE);   // base track info, fQualityFlag bit 48 - unid. hadrons, cuts on TPC comparable to PWGLF cut
    task->AddTrackFilter(QualityCut(isAOD, 1),                  kTRUE);   // base track info, fQualityFlag bit 49 - unid. hadrons, cuts on TPC comparable to PWGLF cut
    task->AddTrackFilter(QualityCut(isAOD, 2),                  kTRUE);   // base track info, fQualityFlag bit 50 - unid. hadrons, cuts on TPC comparable to PWGLF cut
    task->AddTrackFilter(QualityCut(isAOD, 3),                  kTRUE);   // base track info, fQualityFlag bit 51 - unid. hadrons, cuts on TPC comparable to PWGLF cut
}
...
AliAnalysisCuts* JPsiElectronFilterLowMom(Bool_t isAOD) {
  ...
  trackCuts->AddCut(AliDielectronVarManager::kImpactParZ,   -5.0, 5.0);
  trackCuts->AddCut(AliDielectronVarManager::kEta,          -1.0, 1.0);
...
AliAnalysisCuts* QualityCut(Bool_t isAOD, Int_t spec) {
  ...
  // spec = 2 or 3: PWGLF/SPECTRA/ChargedHadrons/MultDepSpec/AliMultDepSpecAnalysisTask.cxx
