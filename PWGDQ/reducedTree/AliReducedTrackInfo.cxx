  ...
  fMCGeneratorIndex           (c.fMCGeneratorIndex)
{
   ...
   for(Int_t i=0; i<4;  ++i) {
    fTPCnSig[i] = c.fTPCnSig[i];
    fTOFnSig[i] = c.fTOFnSig[i];
    fITSnSig[i] = c.fITSnSig[i];
   }
   for(Int_t i=0; i<4;  ++i) {
    fTPCdEdxInfoQmax[i] = c.fTPCdEdxInfoQmax[i];
    fTPCdEdxInfoQtot[i] = c.fTPCdEdxInfoQtot[i];
  }
   for(Int_t i=0; i<6;  ++i) {
    fTrackParam[i] = c.fTrackParam[i];
  }
   for(Int_t i=0; i<21; ++i) {
    fCovMatrix[i] = c.fCovMatrix[i];
  }
   for(Int_t i=0; i<3;  ++i) {
    fMCMom       [i] = c.fMCMom       [i];
    fMCFreezeout [i] = c.fMCFreezeout [i];
  }
   for(Int_t i=0; i<4;  ++i) {
    fMCLabels [i] = c.fMCLabels [i];
    fMCPdg    [i] = c.fMCPdg    [i];
  }
}
