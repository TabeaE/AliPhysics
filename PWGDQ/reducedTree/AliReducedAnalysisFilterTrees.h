...
public:
  ...
  void AddMeasMultTrackCut(AliReducedInfoCut* cut) {fMeasMultTrackCuts.Add(cut);}
  ...
  const Char_t* GetMeasMultcutName(Int_t i) const {return (i<fMeasMultTrackCuts.GetEntries() ? fMeasMultTrackCuts.At(i)->GetName() : "");}
...
protected:
  ...
  TList fMeasMultTrackCuts;  // cuts for tracks used for measured multiplicity estimation
  TList fTrueMultTrackCuts;  // cuts for MC tracks used for true multiplicity estimation
