...
/*
 ...
 AliResonanceFits object is supposed to be configured in a macro and provided to the 
 AliCorrelationExtraction class. A brief overview of the main functionality of the class is provided in 
 the following. For a more detailed description of the different functions check 
 AliCorrelationExtraction.cxx.

 1) Input data ------------------------------------------------------------------------------------

  The main input distributions are the same-event opposite sign (fSEOS) and mixed-event opposite sign 
  (fMEOS) THnF (or THnSparseF) distributions obtained from the reducedTree framework, namely the class
  ...
  Additionally, depending on the correlation signal extraction method that is used, like sign 
  distributions are required. Hereby, correlation distributions (fSEMM, fSEPP, fMEMM and fMEPP) and 
  electron/positron pair distributions (fSEPPPair, fSEMMPair, fMEPPPair and fMEMMPair) are set with the 
  functions SetSELSHistogram(), SetMELSHistogram(), SetSELSPairHistogram() and SetMELSPairHistogram(), 
  respectively.

  All input THnF (or THnSparseF) can also be set at the same time using SetHistograms() for the 
  correlation and SetPairHistograms() for the pair distributions.

  In order to correct the J/psi - hadron correlation function for the associated hadron efficiency, an
  1-dimensional efficiency distribution (fHadronEff) can be set via SetHadronEfficiencyHistogram(). Note 
  that the variable/dimension (see 2) in which the efficiency is provided has to be set.

 2) Dimensions and variables ------------------------------------------------------------------------------------

  There are 3 "types" of variables (THnF/THnSparseF dimensions) that can be (2.1 and 2.3) or need to be 
  (2.2) defined by the user in order to do the correlation signal extraction with this class:

  2.1) General variables, which correspond to the input THnF/THnSparseF dimensions (i.e. axes), can be 
    set using AddVariable() or AddVariables(). Hereby, variable ranges can be set using SetVarRange() in 
    order to cut on the axes of the input distributions. Not all dimensions have to be provided and 
    those which are not provided by the user or are not given a specific variable range will be 
    integrated over.

  2.2) Correlation variables, i.e. the delta phi, delta eta and e+e- pair mass variables, are required 
    for the correlation signal extraction and can be set using SetDeltaPhiVariable(),
    SetDeltaEtaVariable() and SetMassVariable(), respectively. In addition to the specific setting using 
    the 3 functions quoted above, the corresponding dimension of the input THnF/THnSparseF has to be 
    defined using AddVariable() or AddVariables() as described in 2.1. Variable ranges, i.e. cuts, can 
    also be defined for the 3 correlation variables using SetVarRange().
    In addition, if the like sign distributions (see 1) are used for the correlation signal extraction, 
    the index (i.e. the axis) of the mass variable in the like sign electron pair input distributions 
    has to be defined using SetMassVariablePairIndex().

  2.3) Mixing variables, i.e. event variables which are used to define the event mixing pool, can be 
    provided using AddMixingVariable() or AddMixingVariables(). These have to be dimensions of the input 
    THnF/THnSparseF and, if provided, will be used during the calculation of the inclusive e+e- - hadron 
    correlation extraction. Hereby, the inclusive correlation function will be extracted in bins of the 
    mixing variables and summed afterwards.
    Typical examples for these variables are the z_vtx. position or the event multiplicity. These 
    variables are not required for the correlation signal extraction and a test on 2016 pp 13 TeV data 
    showed no significant difference between using the mixing variables (z_vtx, mult, and both) or not.

 3) User options ------------------------------------------------------------------------------------------------

  There are different options for the correlation signal extraction which can be set by the user.

  3.1) The background method, all of which can be found in the enumerated list BackgroundMethods, that 
    is used to determine the non-J/psi background in the e+e- - hadron correlation function can be set 
    via SetBackgroundMethod(). By default, no background extraction method is set and the class will not 
    run the processing.

  3.2) If a fitting method (kBkgFitting) is requested for the background determination, a fit function 
    must be provided via SetBackgroundFitFunction(). There is no default fit function defined. A mass 
    exclusion range that should not be considered for the background fit is required as well as there is 
    no default. It must be set via SetMassExclusionRange().

  3.3) The e+e- pair mass signal window must be defined using SetSignalMassWindow(). Again, there is not 
    default setting and the processing will be stopped if none is provided.

  3.4) The e+e- pair background mass window(s) can be set with SetBackgroundMassWindows() and define the 
    mass windows used for the correlation background extraction with the various background extraction 
    methods. Note that some methods (e.g. kBkgSideband) require only one background window while others 
    (e.g. kBkgFitting or kBkgSuperposition) require more. Check Initialize() or the different background 
    extraction functions in order to find out how many windows you need.

  3.5) Additionally, a verbose mode can be used by calling SetVerbose(). Per default, verbose mode is 
    switched off.

 4) Processing --------------------------------------------------------------------------------------------------

  The processing, i.e. the correlation signal extraction, is done by calling Process() in the user 
  defined macro. This function returns a flag (fProcessDone) that can be tested to see if the processing 
  was successful. The processing works as follows:

  4.1) Initialize() is called first and will stop the processing in case of erroneous inputs or 
    conflicting user options.

  4.2) The J/psi signal is extracted using the user provided AliResonanceFits object.

  4.3) The inclusive e+e- - hadron correlation function is calculated in the user provided mass signal 
    window using the (private) function CalculateInclusiveCorrelation().

  4.4) Depending on the user options for the background extraction method, a specific function to 
    determined the background correlation function (CalculateBackgroundCorrelation...) is called. For 
    some background extraction methods (kBkgSuperposition and kBkgFitting), the signal correlation 
    function (i.e. J/psi - hadron) is already calculated here and 4.5 is skipped.
  ...
  4.6) There are two ways an efficiency correction can be applied, depending on if the J/psi should be 
    efficiency corrected or not:
    1) If no J/psi efficicency correction is required, a hadron efficiency map (1D) can be set using 
    SetHadronEfficiencyHistogram(). The signal correlation will then be corrected for the hadron
    efficiency after the signal extraction has been performed.
    2) In case a J/psi efficiency correction is required in addition to the hadron efficiency 
    correction, the input correlation histograms already have to be corrected for the efficiency using a 
    1/(eff_jpsi x eff_hadron) weight when filling the input histograms. If this is the case, the J/psi 
    efficiency must be provided via SetJpsiEfficiency() in order to properly correct the trigger 
    normalization.

 5) Output ------------------------------------------------------------------------------------------------------

  After the processing is done, the final and some intermediate distributions can be obtained using the
  provided getter functions. The most important output objects hereby are the inclusive (all e+e-), 
  background (non-J/psi e+e-) and signal (J/psi) correlation functions which can be obtained using the 
  GetInclusiveCF...(), GetBackgroundCF...() and GetSignalCF...() functions, respectively. For all other 
  possible output objects, please consult the class in order to understand in detail what they represent.

  Additionally, the trigger (i.e. e+e- pair) values caluclated using the AliResonanceFits object can be 
  obtained in the signal and background windows using GetTriggerValuesSignal() and 
  GetTriggerValuesBackground(), respectively. The trigger values are provided as arrays with the 
  elements according to the enumerated list TriggerValues, including e.g. the signal (J/psi) and 
  background counts.
...
class AliCorrelationExtraction : public TObject {
  ...
  public:
    ...
    void SetHadronEfficiencyHistogram(TH1D* h, Int_t var) {fHadronEff = h;
      fHadronEfficiencyVariable = var; fProcessDone = kFALSE;}
    void SetAliResonanceFitsObject(AliResonanceFits* resonanceFits) {
      fResonanceFits = (AliResonanceFits*)resonanceFits->Clone("ResonanceFits"); fProcessDone = kFALSE;}
    ...
    void SetSignalMassWindow(Double_t min, Double_t max) {fMassSignalRange[0] = min;
                                                          fMassSignalRange[1] = max;
                                                          fProcessDone        = kFALSE;}
    void SetBackgroundMassWindows(Int_t n, Double_t* min, Double_t* max);
    void SetMassExclusionRange(Double_t min, Double_t max) {fMassExclusionRange[0] = min, 
                                                            fMassExclusionRange[1] = max;}
    ...
    const Double_t*   GetTriggerValuesBackground (Int_t massWindow) const {
      return (fProcessDone ? fTrigValBkg[massWindow] : 0x0);}
  ...
  private:
    ...
    Double_t fVarLimits          [kNMaxVariables][2];     // variable limits
                                                          //   (i.e. "cuts" or selection ranges)
    ...
    // mass ranges
    Int_t    fNBackgroundMassRanges;                               // number of bkg mass windows
    Double_t fBackgroundMassRanges[kNMaxBackgroundMassRanges][2];  // bkg mass windows
    ...
    Double_t fTrigValBkg[kNMaxBackgroundMassRanges][kNTriggerValues]; // trigger values bkg mass ranges
    ...
    Bool_t CalculateInclusiveCorrelationInMixingBins (Int_t currentVar, Int_t& nCalls, THnBase* seos, 
                                                      THnBase* meos, TH2D* (&inclCF));
