...
Bool_t AliCorrelationExtraction::NormalizeToNearSidePeak(TH2D* h) {
  ...
  Double_t norm = 0.;
  for(Int_t i=0; i<(Int_t)binNumbers.size(); ++i) norm += h->GetBinContent(binNumbers.at(i));
...
//_______________________________________________________________________________
Bool_t AliCorrelationExtraction::CalculateInclusiveCorrelationInMixingBins(Int_t currentVar,
  Int_t& nCalls, THnBase* seos, THnBase* meos, TH2D* (&inclCF))
{
  //
  ...
  for(Int_t bin=0; bin<fNMixingVarBins[currentVar]; ++bin) {
    if(currentVar<fNMixingVariables-1) {
      ...
      seos->GetAxis(fMixingVarIndices[currentVar])->SetRangeUser(
        fMixingVarBinLimits[currentVar][bin][0]+EPSILON,
        fMixingVarBinLimits[currentVar][bin][1]-EPSILON);
      meos->GetAxis(fMixingVarIndices[currentVar])->SetRangeUser(
        fMixingVarBinLimits[currentVar][bin][0]+EPSILON,
        fMixingVarBinLimits[currentVar][bin][1]-EPSILON);

      // project
      seosTmp = std::unique_ptr<TH2D>(static_cast<TH2D*>(seos->Projection(fDeltaPhiVariableIndex, 
                                                                          fDeltaEtaVariableIndex, 
                                                                          "e")));
      meosTmp = std::unique_ptr<TH2D>(static_cast<TH2D*>(meos->Projection(fDeltaPhiVariableIndex, 
                                                                          fDeltaEtaVariableIndex, 
                                                                          "e")));
...
//_______________________________________________________________________________
Bool_t AliCorrelationExtraction::CalculateInclusiveCorrelation(Double_t minMass,
  Double_t maxMass, Bool_t isSignalRange, TH2D* (&seos), TH2D* (&meos), TH2D* (&inclCF2D),
  TH1D* (&inclCF1D))
{
  //
  ...
  if(fSEOS)
    seostmp = std::unique_ptr<THnF>(static_cast<THnF*>(
      fSEOS->Clone(Form("seostmp_%.6f",gRandom->Rndm()))));
  else if(fSEOSSparse)
    seostmp = std::unique_ptr<THnSparseF>(static_cast<THnSparseF*>(
      fSEOSSparse->Clone(Form("seostmp_%.6f",gRandom->Rndm()))));
  if(fMEOS)
    meostmp = std::unique_ptr<THnF>(static_cast<THnF*>(
      fMEOS->Clone(Form("meostmp_%.6f",gRandom->Rndm()))));
  else if(fMEOSSparse)
    meostmp = std::unique_ptr<THnSparseF>(static_cast<THnSparseF*>(
      fMEOSSparse->Clone(Form("meostmp_%.6f",gRandom->Rndm()))));
  ...
  if(isSignalRange) {
    ...
    fTrigValBkg[massWindow][kSigFrac]    = fTrigValBkg[massWindow][kSig] / 
                                           fTrigValBkg[massWindow][kSplusB];
    ...
    fTrigValBkg[massWindow][kBkgFrac]    = fTrigValBkg[massWindow][kBkg] /
                                           fTrigValBkg[massWindow][kSplusB];
    fTrigValBkg[massWindow][kBkgFracErr] = fTrigValBkg[massWindow][kSigFracErr];
  } else {
    cout << "AliCorrelationExtraction::CalculateInclusiveCorrelation() Fatal: Mass range [" << minMass 
         << ", " << maxMass<< "] not recognized!" << endl;
    return kFALSE;
  }
...
Bool_t AliCorrelationExtraction::HadronEfficiencyCorrection() {
  ...
  if(fVerboseFlag) {
    ...
    for(Int_t i=0; i<(Int_t)normBins.size(); ++i) cout << normBins.at(i) << ", ";
    cout << "}" << endl;
    cout << "AliCorrelationExtraction::HadronEfficiencyCorrection() effBins    = {";
    for(Int_t i=0; i<(Int_t)effBins.size(); ++i) cout << effBins.at(i) << ", ";
    cout << "}" << endl;
  }
  ...
  for(Int_t i=0; i<(Int_t)commonBins.size(); ++i) newBins[i] = commonBins.at(i);
...
void AliCorrelationExtraction::PrintUserOptions() {
  ...
  //
  cout << endl;
  cout << "AliCorrelationExtraction summary of all user options ======================================" 
       << endl;
  ...
  if(fUseMixingVars) {
    ...
    for(Int_t i=0; i<fNMixingVariables; ++i) {
      ...
      for(Int_t j=0; j<fNMixingVarBins[i]; ++j) {
        cout << "\tbin " << j << " range = [" << fMixingVarBinLimits[i][j][0] << ", "
             << fMixingVarBinLimits[i][j][1] << "]" << endl;
      }
    }
  }
  cout << "fMassVariable              = " << AliReducedVarManager::fgVariableNames[fMassVariable]
       << ",\tindex = " << fMassVariableIndex << endl;
  cout << "fDeltaPhiVariable          = " << AliReducedVarManager::fgVariableNames[fDeltaPhiVariable] 
       << ",\tindex = " << fDeltaPhiVariableIndex << endl;
  cout << "fDeltaEtaVariable          = " << AliReducedVarManager::fgVariableNames[fDeltaEtaVariable] 
       << ",\tindex = " << fDeltaEtaVariableIndex << endl;
  if(fHadronEff)
    cout << "fHadronEfficiencyVariable  = "
         << AliReducedVarManager::fgVariableNames[fHadronEfficiencyVariable] << ",\tindex = "
         << fHadronEfficiencyVariableIndex << endl;

  if(fMassVariableIndexPair >= 0)
    cout << "LS pair mass variable set: index = " << fMassVariableIndexPair << endl;
  cout << "general user options: +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" 
       << endl;
  ...
  if(fBkgFitFunction)
    cout << "fBkgFitFunction    = " << fBkgFitFunction << " (" << fBkgFitFunction->GetName() << ")"
         << endl;
  cout << "mass ranges: ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" 
       << endl;
  cout << "signal mass range      = [" << fMassSignalRange[0] << ", " << fMassSignalRange[1] << "]"
       << endl;
  cout << "fNBackgroundMassRanges = " << fNBackgroundMassRanges << endl;
  for(Int_t i=0; i<fNBackgroundMassRanges; ++i) {
    cout << "background mass range " << i << "  = [" << fBackgroundMassRanges[i][0] << ", "
         << fBackgroundMassRanges[i][1] << "]" << endl;
  }
  cout << "===========================================================================================" 
       << endl;
  cout << endl;
