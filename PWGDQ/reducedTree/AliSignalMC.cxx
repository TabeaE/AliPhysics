...
AliSignalMC::AliSignalMC(const Char_t* name, const Char_t* title, Int_t nProngs/*=1*/,
                                  ...
                                  UInt_t excludeSources[]/*=0x0*/, Bool_t useANDonSourceBits[]/*=0x0*/)
{
  ...
  for(UInt_t i=0; i<fNGenerations; ++i) {
    ...
    if(excludePDG)         fExcludePDG           [prong][i] = excludePDG[i];
    if(excludeSources)     fExcludeSource        [prong][i] = excludeSources[i];
    if(useANDonSourceBits) fUseANDonSourceBitMap [prong][i] = useANDonSourceBits[i];
  }
}
                               ...
                               Bool_t exclude/*=kFALSE*/)
{
  ...
  // NOTE: The exclude factor is set here via the inidividual source bit, but actually overrides the 
  //       flag for all sources (in case multiple sources have been defined)
...
Bool_t AliSignalMC::TestPDG(UInt_t prong, UInt_t generation, Int_t pdg) {
  ...
  switch(TMath::Abs(fPDGcodes[prong][generation])) {
    ...
    case 100:     // light flavoured mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=100 && absPDG<=199;
      else {
    ...
    case 1000:     // light flavoured baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=1000 && absPDG<=1999;
      else {
    ...
    case 200:     // light flavoured mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=200 && absPDG<=299;
      else {
    ...
    case 2000:     // light flavoured baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=2000 && absPDG<=2999;
      else {
    ...
    case 300:     // all strange mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=300 && absPDG<=399;
      else {
    ...
    case 3000:     // all strange baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=3000 && absPDG<=3999;
      else {
    ...
    case 400:     // all charmed mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=400 && absPDG<=499;
      else {
    ...
    case 401:     // open charm mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=400 && absPDG<=439;
      else {
    ...
    case 402:     // open charm mesons and baryons together
      if(fCheckBothCharges[prong][generation])
        ...
        if(fPDGcodes[prong][generation] > 0)
    ...
    case 404:     // charged open charmed mesons NO s quark
      if(fCheckBothCharges[prong][generation]) decision = (absPDG>=410 && absPDG<=419);
      else {
    ...
    case 405:     // neutral open charmed mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=420 && absPDG<=429;
      else {
    ...
    case 406:     // charged open charmed mesons with s quark
      if(fCheckBothCharges[prong][generation]) decision = (absPDG>=430 && absPDG<=439);
      else {
    ...
    case 4000:     // all charmed baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=4000 && absPDG<=4999;
      else {
    ...
    case 4001:     // open charm baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=4000 && absPDG<=4399;
      else {
    ...
    case 500:      // all beauty mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=500 && absPDG<=599;
      else {
    ...
    case 501:      // open beauty mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=500 && absPDG<=549;
      else {
    ...
    case 504:     // neutral open beauty mesons NO s quark
      if(fCheckBothCharges[prong][generation]) decision = (absPDG>=510 && absPDG<=519);
      else {
    ...
    case 505:     // charged open beauty mesons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=520 && absPDG<=529;
      else {
    ...
    case 506:     // charged open beauty mesons with s quark
      if(fCheckBothCharges[prong][generation]) decision = (absPDG>=530 && absPDG<=539);
      else {
    ...
    case 5000:      // all beauty baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=5000 && absPDG<=5999;
      else {
    ...
    case 5001:      // open beauty baryons
      if(fCheckBothCharges[prong][generation]) decision = absPDG>=5000 && absPDG<=5499;
      else {
