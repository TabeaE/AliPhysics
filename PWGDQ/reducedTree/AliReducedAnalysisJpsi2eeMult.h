...
protected:
   ...
   Bool_t fOptionRunOverMC;           // true: trees contain MC info -> fill histos to compute 
                                      //       efficiencies, false: run normally as on data
   Bool_t fOptionRunLikeSignPairing;  // true (default): performs the like sign pairing in addition to 
                                      //                 the opposite pairing
   Bool_t fOptionLoopOverTracks;      // true (default); if false do not loop over tracks and 
                                      //                 consequently no pairing
