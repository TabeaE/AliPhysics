...
void AliAnalysisTaskReducedTreeMaker::UserCreateOutputObjects()
{
  ...
  if(fTreeWritingOption==kBaseEventsWithBaseTracks || fTreeWritingOption==kFullEventsWithBaseTracks) {
    for(UInt_t i=0; i<fWriteBaseTrack.size(); i++) {
      if(!fWriteBaseTrack.at(i)) {
        printf("AliAnalysisTaskReducedTreeMaker::UserCreateOutputObjects() WARNING: Full tracks requested for "
          "filter %d, but interferes with fTreeWritingOption choice! Only base tracks will be written. \n", i);
  ...
  if(fTreeWritingOption==kBaseEventsWithBaseTracks || fTreeWritingOption==kFullEventsWithBaseTracks) {
    for(Int_t i=0; i<kMaxMCsignals; i++) {
      if(fMCsignalsWritingOptions[i] == kFullTrack) {
        printf("AliAnalysisTaskReducedTreeMaker::UserCreateOutputObjects() WARNING: Full tracks requested for "
        "MC signal %d, but interferes with fTreeWritingOption choice! Only base tracks will be written. \n", i);
  ...
  const Char_t* offlineTRDTriggerNames[kNOfflineTRDtrigNames] = {"Total", "No Phys Sel", "HQU | HSE", 
                                                                 "HQU", "HSE", "Nuclei", "Jet"};
  fTRDEventsHistogram = new TH2I("TRDEventStatistics", "TRD Event statistics",
                                 kNEventSelectionNames, -0.5, -0.5+kNEventSelectionNames,
                                 kNOfflineTRDtrigNames, -2.5, -2.5+kNOfflineTRDtrigNames);
  for(Int_t i=1; i<=kNOfflineTRDtrigNames; ++i)
    fTRDEventsHistogram->GetYaxis()->SetBinLabel(i, offlineTRDTriggerNames[i-1]);
  for(Int_t i=1; i<=kNEventSelectionNames; ++i)
    fTRDEventsHistogram->GetXaxis()->SetBinLabel(i, selectionNames[i-1]);
  ...
  const Char_t* offlineEMCalTriggerNames[kNOfflineEMCALtrigNames] = {"Total", "No Phys Sel",
    "EMC7 | DMC7", "EMC7", "DMC7", "EMC1 | DMC1", "EMC1", "DMC1", "EMCEGA", "EG1 | DG1", "EG2 | DG2", 
    "EG1", "EG2", "DG1", "DG2", "EMCEJE", "EJ1 | DJ1", "EJ2 | DJ2", "EJ1", "EJ2", "DJ1", "DJ2"};
  fEMCalEventsHistogram = new TH2I("EMCalEventStatistics", "EMCal Event statistics",
                                   kNEventSelectionNames,   -0.5, -0.5+kNEventSelectionNames,
                                   kNOfflineEMCALtrigNames, -2.5, -2.5+kNOfflineEMCALtrigNames);
  for(Int_t i=1; i<=kNOfflineEMCALtrigNames; ++i)
    fEMCalEventsHistogram->GetYaxis()->SetBinLabel(i, offlineEMCalTriggerNames[i-1]);
  for(Int_t i=1; i<=kNEventSelectionNames; ++i)
    fEMCalEventsHistogram->GetXaxis()->SetBinLabel(i, selectionNames[i-1]);
  ...
  for(Int_t i=0; i<nCentEstimators; ++i) {
    centEventsHistogram[i] = new TH2I(estimatorNames[i], Form("%s estimator",estimatorNames[i]),
                                      kNEventSelectionNames, -0.5, -0.5+kNEventSelectionNames,
                                      120, -5., 115.);
  ...
  for(Int_t i=0; i<fTrackFilter.GetEntries(); i++)
    fTracksHistogram->GetXaxis()->SetBinLabel(i+2,
      Form("%s",((AliAnalysisCuts*)fTrackFilter.At(i))->GetName()));
  ...
  // kNOfflineTriggerAliases-2 because we do not need "Total" and "No Phys Sel"
  ...
  for(Int_t i=1; i<=fMCsignals.GetEntries(); ++i) {
    ...
    fMCSignalsHistogram->GetXaxis()->SetBinLabel(i,
      Form("%s (%s)",((AliSignalMC*)fMCsignals.At(i-1))->GetName(),trackTypeStr.Data()));
  }
...
void AliAnalysisTaskReducedTreeMaker::UserExec(Option_t *option)
{
  ...
  for(Int_t i=0; i<nCentEstimators; ++i) {
    percentileEstimators[i] = 
      (isOldCent ? centrality   ->GetCentralityPercentile  (Form("%s",estimatorNames[i])) 
...
//             fMCPlpEventsHistogram->Fill(3);
  ...
  // NOTE: If physics selection was not applied (as requested by user) then we can still have events 
  //       with PS not fulfilled.
  FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 1., 
  ...
  if(isPhysAndTrigSel) {
    ...
    FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 4., 
                             percentileEstimators, nCentEstimators);
    PostData(3, fEventsList);
    return;
  }
  ...
  if(fRejectPileup && InputEvent()->IsPileupFromSPD(3,0.8,3.,2.,5.)) {
    FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 6., 
                             percentileEstimators, nCentEstimators);
    PostData(3, fEventsList);
    return;
  } else {
    // Accepted pileup selection
    FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 5., 
                             percentileEstimators, nCentEstimators);
  }
  ...
  if(fEventFilter && !fEventFilter->IsSelected(InputEvent())) {
    ...
    FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 8., 
                             percentileEstimators, nCentEstimators);
    PostData(3, fEventsList);
    return;
  } else {
    FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 7., 
                             percentileEstimators, nCentEstimators);
  }

  // Apply the time range cut needed to reject events with bad TPC pid in some chambers
  // For details see:
  // https://indico.cern.ch/event/830757/contributions/3479738/attachments/1873483/3083952/IArsene_DPG_2019July3.pdf
  ...
  FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype,  emcaltrgtype, 9., percentileEstimators,
                           nCentEstimators);
  ...
  if(fWriteTree) {
    ...
    if(gRandom->Rndm() < fScaleDownEvents) {
      ...
      FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype, emcaltrgtype, 12.0,
                               percentileEstimators, nCentEstimators);
    }
    ...
    for(Int_t i=0; i<fTrackFilter.GetEntries(); ++i) {
      if(fMinSelectedTracks[i]>=0 &&
         (fNSelectedFullTracks[i]+fNSelectedBaseTracks[i])<fMinSelectedTracks[i]) 
      {
        trackFilterAccepted = kFALSE;
        break;
      }
      if(fMaxSelectedTracks[i]>=0 &&
         (fNSelectedFullTracks[i]+fNSelectedBaseTracks[i])>fMaxSelectedTracks[i])
      {
        trackFilterAccepted = kFALSE;
        break;
      }
    ...
    if(trackFilterAccepted) {
      FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype, emcaltrgtype, 11.,
                               percentileEstimators, nCentEstimators);
    }
    ...
    if(fWriteEventsWithOnlyV0s && hasV0s) {
      FillStatisticsHistograms(Bool_t(isPhysSel), isPhysSel, trdtrgtype, emcaltrgtype, 13.,
                               percentileEstimators, nCentEstimators);
    }
                                                     ...
                                                     Int_t minSel/*=-1*/, Int_t maxSel/*=-1*/)
{
  ...
  if(fTrackFilter.GetEntries() < kNMaxTrackFilters) {
    ...
    printf("AliAnalysisTaskReducedTreeMaker::AddTrackFilter() WARNING: Track filter list full (%d entries), "
           "will not add another filter!\n", fTrackFilter.GetEntries());
  }
}
...
void AliAnalysisTaskReducedTreeMaker::AddCaloClusterFilter(AliAnalysisCuts* const filter)
{
  ...
  else printf("AliAnalysisTaskReducedTreeMaker::AddCaloClusterFilter() WARNING: Cluster filter list full "
              "(%d entries), will not add another filter!\n", fClusterFilter.GetEntries());
}

//_________________________________________________________________________________
Bool_t AliAnalysisTaskReducedTreeMaker::IsTrackSelected(AliVParticle* track, Double_t* values,
                                                        std::vector<Bool_t>& filterDecision)
{
  //
  ...
  for(Int_t i=0; i<fTrackFilter.GetEntries(); i++) {
    ...
    if(filter->IsA() == AliDielectronCutGroup::Class())
      filterIsSelected = (dynamic_cast<AliDielectronCutGroup*>(filter))->IsSelected(track, values);
    else if(filter->IsA() == AliDielectronVarCuts::Class())
      filterIsSelected = (dynamic_cast<AliDielectronVarCuts*>(filter))->IsSelected(values);
    else
      filterIsSelected = filter->IsSelected(track);
    if(filterIsSelected) {
...
//_________________________________________________________________________________
Bool_t AliAnalysisTaskReducedTreeMaker::IsClusterSelected(AliVCluster* cluster,
                                                          std::vector<Bool_t>& filterDecision)
{
  //
...
//_________________________________________________________________________________
void AliAnalysisTaskReducedTreeMaker::SetTrackFilterQualityFlags(AliReducedBaseTrack* track,
                                                                 std::vector<Bool_t> filterDecision)
{
  //
  ...
  //
  for(Int_t i=0; i<fTrackFilter.GetEntries(); i++) {
    if(filterDecision[i]) track->SetQualityFlag(32+i);  // AliReduceBaseTrack::fQualityFlags BIT 32+i
                                                        // (0<=i<fTrackFilter.GetEntries())
  }
}
...
void AliAnalysisTaskReducedTreeMaker::FillTrackStatisticsHistogram()
{
  ...
  fTracksHistogram->Fill(-1.0, 0.0,
                         fReducedEvent->fTracks->GetEntries()+fReducedEvent->fTracks2->GetEntries());
  if(fTreeWritingOption==kBaseEventsWithBaseTracks || fTreeWritingOption==kFullEventsWithBaseTracks) {
  ...
  // counters for tracks belonging to V0s (first 4 elements), on the fly V0 pairs (next 4 elements) and
  //   offline V0 pairs (next 4 elements)
  for(Int_t i=0; i<4; i++) {
    fTracksHistogram->Fill(fTrackFilter.GetEntries()+1+i, 0.0,
      fNSelectedFullTracks[fTrackFilter.GetEntries()+i]+fNSelectedBaseTracks[fTrackFilter.GetEntries()+i]);
    fTracksHistogram->Fill(fTrackFilter.GetEntries()+1+i, 1.0,
      fNSelectedFullTracks[fTrackFilter.GetEntries()+i]);
    fTracksHistogram->Fill(fTrackFilter.GetEntries()+1+i, 2.0,
      fNSelectedBaseTracks[fTrackFilter.GetEntries()+i]);
  }
  // Counter for the total number of V0 pairs selected
  // (also written if the fCandidates branch is not switched off)
  fTracksHistogram->Fill(fTrackFilter.GetEntries()+5, 0.0, fReducedEvent->fCandidates->GetEntries());
  for(Int_t i=0; i<8; i++) {
    fTracksHistogram->Fill(fTrackFilter.GetEntries()+6+i, 0.0,
                           fNSelectedFullTracks[fTrackFilter.GetEntries()+4+i]);
  }
  // MC tracks counters
  fTracksHistogram->Fill(fTrackFilter.GetEntries(), 0.0,
    fNSelectedFullTracks[fTrackFilter.GetEntries()+12]+fNSelectedBaseTracks[fTrackFilter.GetEntries()+4]);
...
void AliAnalysisTaskReducedTreeMaker::FillEventInfo()
{
  ...
  if(fUseAnalysisUtils) {
    ...
    fAnalysisUtils->SetMinPlpContribMV(5);
    fAnalysisUtils->SetMaxPlpChi2MV(5.);
  ...
  if(fTreeWritingOption==kBaseEventsWithBaseTracks || fTreeWritingOption==kBaseEventsWithFullTracks) return;
  ...
  if(multSelection) {
    ...
    estimator = multSelection->GetEstimator("OnlineV0M");
    if(estimator) eventInfo->fMultiplicityEstimators[0]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("OnlineV0A");
    if(estimator) eventInfo->fMultiplicityEstimators[1]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("OnlineV0C");
    if(estimator) eventInfo->fMultiplicityEstimators[2]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("ADM");
    if(estimator) eventInfo->fMultiplicityEstimators[3]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("ADA");
    if(estimator) eventInfo->fMultiplicityEstimators[4]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("ADC");
    if(estimator) eventInfo->fMultiplicityEstimators[5]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("SPDClusters");
    if(estimator) eventInfo->fMultiplicityEstimators[6]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("SPDTracklets");
    if(estimator) eventInfo->fMultiplicityEstimators[7]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("RefMult05");
    if(estimator) eventInfo->fMultiplicityEstimators[8]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("RefMult08");
    if(estimator) eventInfo->fMultiplicityEstimators[9]  = estimator->GetValue();
    estimator = multSelection->GetEstimator("V0M");
    if(estimator) eventInfo->fMultiplicityEstimators[10] = estimator->GetValue();
    estimator = multSelection->GetEstimator("V0A");
    if(estimator) eventInfo->fMultiplicityEstimators[11] = estimator->GetValue();
    estimator = multSelection->GetEstimator("V0C");
    if(estimator) eventInfo->fMultiplicityEstimators[12] = estimator->GetValue();
  }
  ...
  if(eventVtxSPD) {
    eventInfo->fVtxSPD[0] = (isESD ? ((AliESDVertex*)eventVtxSPD)->GetX()
                                   : ((AliAODVertex*)eventVtxSPD)->GetX());
    eventInfo->fVtxSPD[1] = (isESD ? ((AliESDVertex*)eventVtxSPD)->GetY()
                                   : ((AliAODVertex*)eventVtxSPD)->GetY());
    eventInfo->fVtxSPD[2] = (isESD ? ((AliESDVertex*)eventVtxSPD)->GetZ()
                                   : ((AliAODVertex*)eventVtxSPD)->GetZ());
  ...
  if(isESD) {
    ...
    if(tzero) {
      ...
      for(Int_t i=0; i<24; i++) eventInfo->fT0amplitude[i] = tzero->GetT0amplitude()[i];
      for(Int_t i=0; i<3;  i++) eventInfo->fT0TOF[i]       = tzero->GetT0TOF()[i];
      for(Int_t i=0; i<3;  i++) eventInfo->fT0TOFbest[i]   = tzero->GetT0TOFbest()[i];
  ...
  if(isAOD) {
    ...
    if(tzero) {
      ...
      eventInfo->fT0zVertex = tzero->GetT0zVertex();
      for(Int_t i=0; i<26; i++) eventInfo->fT0amplitude[i] = tzero->GetAmp(i);
      for(Int_t i=0; i<3;  i++) eventInfo->fT0TOF[i]       = tzero->GetT0TOF()[i];
      for(Int_t i=0; i<3;  i++) eventInfo->fT0TOFbest[i]   = tzero->GetT0TOFbest()[i];
      eventInfo->fT0pileup    = tzero->GetPileupFlag();
      eventInfo->fT0sattelite = tzero->GetSatellite();
  ...
  for(Int_t i=0; i<64; ++i) eventInfo->fVZEROMult[i] = vzero->GetMultiplicity(i);
...
void AliAnalysisTaskReducedTreeMaker::FillCaloClusters()
{
  ...
  for(Int_t iclus=0; iclus<nclusters; ++iclus) {
    ...
    if(fClusterFilter.GetEntries() > 0)
      clusterFilterDecision = IsClusterSelected(cluster, individualFilterDecisions);
    ...
    reducedCluster->fType      = (cluster->IsEMCAL() ? AliReducedCaloClusterInfo::kEMCAL
                                                     : AliReducedCaloClusterInfo::kPHOS);
...
//_________________________________________________________________________________
Bool_t AliAnalysisTaskReducedTreeMaker::CheckParticleSource(AliMCEvent* event, Int_t ipart,
                                                            AliSignalMC* mcSignal)
{
  //
  ...
  //         For a given generation, all the sources for which corresponding bits are enabled,
  //         must be fulfilled
  //
  if(mcSignal->GetNProngs() > 1) return kFALSE;
  ...
  for(UInt_t ig=0; ig<mcSignal->GetNGenerations(); ++ig) {
    ...
    if(!currentGenerationParticle)  return kFALSE;  // if there are sources requested, but MC history 
                                                    // finished, evaluate to FALSE
    ...
    if(mcSignal->CheckSourceBit(0, ig, AliSignalMC::kPhysicalPrimary)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kPhysicalPrimary) !=
         event->IsPhysicalPrimary(currentGenerationLabel))
      {decision |= (UInt_t(1) << AliSignalMC::kPhysicalPrimary);}
    }
    if(mcSignal->CheckSourceBit(0, ig, AliSignalMC::kFromBGEvent)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kFromBGEvent) !=
         event->IsFromBGEvent(currentGenerationLabel))
      {decision |= (UInt_t(1) << AliSignalMC::kFromBGEvent);}
    }
    if(mcSignal->CheckSourceBit(0, ig, AliSignalMC::kSecondaryFromWeakDecay)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kSecondaryFromWeakDecay) !=
         event->IsSecondaryFromWeakDecay(currentGenerationLabel))
      {decision |= (UInt_t(1) << AliSignalMC::kSecondaryFromWeakDecay);}
    }
    if(mcSignal->CheckSourceBit(0, ig, AliSignalMC::kSecondaryFromMaterial)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kSecondaryFromMaterial) !=
         event->IsSecondaryFromMaterial(currentGenerationLabel))
      {decision |= (UInt_t(1) << AliSignalMC::kSecondaryFromMaterial);}
    }
    if(mcSignal->CheckSourceBit(0, ig, AliSignalMC::kFromSubsidiaryEvent)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kFromSubsidiaryEvent) !=
         event->IsFromSubsidiaryEvent(currentGenerationLabel))
      {decision |= (UInt_t(1) << AliSignalMC::kFromSubsidiaryEvent);}
    }
    if(mcSignal->CheckSourceBit(0, ig, AliSignalMC::kRadiativeDecay)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kRadiativeDecay) !=
         (currentGenerationParticle->GetNDaughters()>2))
    ...
    if(mcSignal->GetUseANDonSourceBits(0,ig) && (decision != mcSignal->GetSources(0,ig)))
      return kFALSE;  // not all req sources are fullfilled
...
//_________________________________________________________________________________
Bool_t AliAnalysisTaskReducedTreeMaker::CheckParticleSourceAOD(AliMCEvent* event, Int_t ipart,
                                                               AliSignalMC* mcSignal)
{
  //
  // Check that the particle satisfies the source criteria specified in the mcSignal
  ...
  for(UInt_t ig=0; ig<mcSignal->GetNGenerations(); ++ig) {
    ...
    if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kPhysicalPrimary)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kPhysicalPrimary) !=
         currentGenerationParticle->IsPhysicalPrimary())
        decision |= (UInt_t(1) << AliSignalMC::kPhysicalPrimary);
    }
    if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kFromBGEvent)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kFromBGEvent) !=
    ...
    if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kSecondaryFromWeakDecay)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kSecondaryFromWeakDecay) !=
         currentGenerationParticle->IsSecondaryFromWeakDecay())
        decision |= (UInt_t(1) << AliSignalMC::kSecondaryFromWeakDecay);
    }
    if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kSecondaryFromMaterial)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kSecondaryFromMaterial) !=
         currentGenerationParticle->IsSecondaryFromMaterial())
        decision |= (UInt_t(1) << AliSignalMC::kSecondaryFromMaterial);
    }
    if(mcSignal->CheckSourceBit(0,ig, AliSignalMC::kFromSubsidiaryEvent)) {
      if(mcSignal->GetSourceExclude(0,ig,AliSignalMC::kFromSubsidiaryEvent) !=
...
UInt_t AliAnalysisTaskReducedTreeMaker::MatchMCsignals(Int_t iparticle, Bool_t isAOD)
{
  ...
  for(Int_t isig=0; isig<nMCsignals; ++isig) {
    ...
    if(mcMatch) mcSignalsMap |= (UInt_t(1)<<isig);
  }
  return mcSignalsMap;
}
...
void AliAnalysisTaskReducedTreeMaker::FillMCTruthInfo(Bool_t isAOD)
{
  ...
  for(Int_t i=0; i<event->GetNumberOfTracks(); ++i) {
    ...
    writeBaseTrack = CheckMCtruthWriteFormat(mcSignalsMap);  // check which track format (base/full)
                                                             // should be used
    // write the track in the first track array if the format is full track
    // if the track format is base track then write it on either the first or the second array,
    // depending on the tree writing options
...
void AliAnalysisTaskReducedTreeMaker::FillTrackInfo()
{
  ...
  for(Int_t iv0=0; iv0<fReducedEvent->fNV0candidates[1]; ++iv0) {
    ...
    leg1Found[pairId] = kFALSE;
    leg2Found[pairId] = kFALSE;
    ...
    if(!leg1Found[pairId]) {
      trackIdsV0[pairId][nV0LegsTagged[pairId]] = pair->fLegIds[0];
      ++nV0LegsTagged[pairId];
    }
    if(!leg2Found[pairId]) {
      trackIdsV0[pairId][nV0LegsTagged[pairId]] = pair->fLegIds[1];
      ++nV0LegsTagged[pairId];
    }
    ...
    if(isPureV0) {
      ...
      if(!leg1Found[pairId]) {
        trackIdsPureV0[pairId][nPureV0LegsTagged[pairId]] = pair->fLegIds[0];
        ++nPureV0LegsTagged[pairId];
      }
      if(!leg2Found[pairId]) {
        trackIdsPureV0[pairId][nPureV0LegsTagged[pairId]] = pair->fLegIds[1];
        ++nPureV0LegsTagged[pairId];
      }
    }
  }
    ...
    for(Int_t itrackTRD=0; itrackTRD<event->GetNumberOfTrdTracks(); ++itrackTRD) {
      if(isESD) {
        ...
        if(!found) {
          ...
          if((trdTrack->GetLayerMask()&fTRDtrglayerMaskEl) != fTRDtrglayerMaskEl)
            trackTRDGTUlayermask[nTracksTRD] = 0;
          else
            trackTRDGTUlayermask[nTracksTRD] = 1;
  ...
  for(Int_t itrack=0; itrack<ntracks; ++itrack) {
    ...
    if(fTreeWritingOption==kFullEventsWithBaseTracks || fTreeWritingOption==kFullEventsWithFullTracks) {
      ...
      if(fFillEventPlaneInfo) {
        if(!fFlowTrackFilter || (fFlowTrackFilter && fFlowTrackFilter->IsSelected(particle))) {
          ...
          localQvec[0][0] = x;
          localQvec[0][1] = y;
          localQvec[1][0] = (2.0*TMath::Power(x,2.0)-1);
          localQvec[1][1] =  2.0*x*y;
          localQvec[2][0] = (4.0*TMath::Power(x,3.0)-3.0*x);
          localQvec[2][1] = (3.0*y-4.0*TMath::Power(y,3.0));
          for(Int_t iharmonic=0; iharmonic<=2; iharmonic++) {
            for(Int_t icomp=0; icomp<=1; icomp++) {
              evPlane->fQvector[AliReducedEventPlaneInfo::kTPC][iharmonic][icomp] +=
                                                      localQvec[iharmonic][icomp];
              evPlane->fQvector[AliReducedEventPlaneInfo::kTPCptWeights][iharmonic][icomp] +=
                     (particle->Pt()<2.0?particle->Pt():2.0) * localQvec[iharmonic][icomp];
              if(particle->Charge() < 0.0) {
                evPlane->fQvector[AliReducedEventPlaneInfo::kTPCneg][iharmonic][icomp] +=
                                                           localQvec[iharmonic][icomp];
              } if(particle->Charge() > 0.0) {
                evPlane->fQvector[AliReducedEventPlaneInfo::kTPCpos][iharmonic][icomp] +=
                                                           localQvec[iharmonic][icomp];
              }
              if(particle->Eta() < -0.5*fEventPlaneTPCetaGap) {
                evPlane->fQvector[AliReducedEventPlaneInfo::kTPCsideC][iharmonic][icomp] +=
                                                             localQvec[iharmonic][icomp];
              } if(particle->Eta() > 0.5*fEventPlaneTPCetaGap) {
                evPlane->fQvector[AliReducedEventPlaneInfo::kTPCsideA][iharmonic][icomp] +=
                                                             localQvec[iharmonic][icomp];
              }
            }
          }
    ...
    if((isESD && !esdTrack->IsOn(0x1)) || (!isESD && !aodTrack->IsOn(0x1))) {
      ...
      if(TMath::Abs(dcaXY)<3.0 && TMath::Abs(dcaZ)>4.0) {
        ...
        if(TMath::Abs(dcaZ) > 10.0) {
          if(tgl >  0.1)
            pileupTrackArrayP2[++pileupCounterP2] = (isESD ? esdTrack->GetZ() : aodTrack->GetZ());
          if(tgl < -0.1)
            pileupTrackArrayM2[++pileupCounterM2] = (isESD ? esdTrack->GetZ() : aodTrack->GetZ());
        }
      }
    }
    ...
    if(fTrackFilter.GetEntries() > 0)
      trackFilterDecision = IsTrackSelected(particle, values, individualFilterDecisions);
    ...
    Bool_t fSelectedTrackIsBaseTrack = IsSelectedTrackRequestedBaseTrack(individualFilterDecisions,
                                                                         usedForV0Or);
    TClonesArray& tracks = (fWriteSecondTrackArray && fSelectedTrackIsBaseTrack) ? *(fReducedEvent->fTracks2)
                                                                                 : *(fReducedEvent->fTracks);
    AliReducedBaseTrack* reducedParticle = NULL;
    ...
    reducedParticle->PtPhiEta(values[AliDielectronVarManager::kPt], values[AliDielectronVarManager::kPhi],
                              values[AliDielectronVarManager::kEta]);
    ...
    if(isESD) {
      ...
      for(Int_t idx=0; idx<3; ++idx) if(esdTrack->GetKinkIndex(idx)>0)
        reducedParticle->fQualityFlags |= (ULong_t(1)<<(5+idx));
      for(Int_t idx=0; idx<3; ++idx) if(esdTrack->GetKinkIndex(idx)<0)
        reducedParticle->fQualityFlags |= (ULong_t(1)<<(12+idx));
      if(((AliESDVertex*)eventVtx)->UsesTrack(esdTrack->GetID()))
        reducedParticle->fQualityFlags |= (ULong_t(1)<<27);
    }
    if(isAOD) {
      reducedParticle->fTrackId = aodTrack->GetID();
      for(Int_t idx=0; idx<3; ++idx)
        if(aodTrack->GetKinkIndex(idx)>0) reducedParticle->fQualityFlags |= (ULong_t(1)<<(5+idx));
      for(Int_t idx=0; idx<3; ++idx)
        if(aodTrack->GetKinkIndex(idx)<0) reducedParticle->fQualityFlags |= (ULong_t(1)<<(12+idx));
      for(Int_t idx=0; idx<11; ++idx)
        if(aodTrack->TestFilterBit(BIT(idx))) reducedParticle->SetQualityFlag(15+idx);
      if(((AliAODVertex*)eventVtx)->HasDaughter(aodTrack))
        reducedParticle->fQualityFlags |= (ULong_t(1)<<27);
    }
    ...
    trackInfo->fTOFtime              = values[AliDielectronVarManager::kTOFsignal] -
                                       pidResponse->GetTOFResponse().GetTimeZero();
    ...
    if(isESD) {
      ...
      if(tpcInner) {
        ...
        trackInfo->fHelixCenter[0] = helixinfo[5] + (TMath::Cos(helixinfo[2]) * TMath::Abs(1./helixinfo[4]) *
          copysignf(1.0,InputEvent()->GetMagneticField()*values[AliDielectronVarManager::kCharge]));
        trackInfo->fHelixCenter[1] = helixinfo[0] + (TMath::Sin(helixinfo[2]) * TMath::Abs(1./helixinfo[4]) *
          copysignf(1.0,InputEvent()->GetMagneticField()*values[AliDielectronVarManager::kCharge]));
      ...
      if(fFillMCInfo && hasMC) {
        ...
        if(truthParticle) {
          ...
          if(grandmotherTruth)
            grandgrandmotherTruth = AliDielectronMC::Instance()->GetMCTrackMother(grandmotherTruth);
          ...
          if(fFillHFInfo)
            trackInfo->fHFProc = AliDielectronMC::Instance()->GetHFProcess(truthParticle->GetLabel());
        }
      }
    }  // end if(isESD)
    if(isAOD) {
      ...
      if(fFillMCInfo && hasMC) {
        ...
        if(truthParticle) {
          trackInfo->fMCFlags = MatchMCsignals(aodTrack->GetLabel(), kTRUE);  // check which MC signals
                                                                              // match this particle
                                                                              // and fill the bit map
  ...
  if(fTreeWritingOption==kFullEventsWithBaseTracks || fTreeWritingOption==kFullEventsWithFullTracks) {
    ...
    eventInfo->fTPCpileupZ[1] = (pileupCounterM>0 ? -1.0*TMath::Median(pileupCounterM, pileupTrackArrayM)
                                                  : 0.0);
    ...
    eventInfo->fTPCpileupZ2[1] = (pileupCounterM2>0 ? -1.0*TMath::Median(pileupCounterM2, pileupTrackArrayM2)
                                                    : 0.0);
...
void AliAnalysisTaskReducedTreeMaker::FillV0PairInfo()
{
  ...
  for(Int_t iV0=0; iV0<InputEvent()->GetNumberOfV0s(); ++iV0) {  // loop over V0s
    ...
    pdgP  = 0;
    pdgN  = 0;
    Bool_t goodK0s     = kFALSE;
    Bool_t goodLambda  = kFALSE;
    Bool_t goodALambda = kFALSE;
    Bool_t goodGamma   = kFALSE;
    if(fV0OpenCuts) {
      ...
      if(processV0 && TMath::Abs(pdgV0)==310 && TMath::Abs(pdgP)==211 && TMath::Abs(pdgN)==211) {
        goodK0s = kTRUE;
        if(fK0sPionCuts && (!fK0sPionCuts->IsSelected(legPos) || !fK0sPionCuts->IsSelected(legNeg)))
          goodK0s = kFALSE;
      }
      if(processV0 && pdgV0==3122 && (TMath::Abs(pdgP)==211 || TMath::Abs(pdgP)==2212) &&
        (TMath::Abs(pdgN)==211 || TMath::Abs(pdgN)==2212))
      {
        goodLambda = kTRUE;
      ...
      if(processV0 && TMath::Abs(pdgV0)==22 && TMath::Abs(pdgP)==11 && TMath::Abs(pdgN)==11) {
        goodGamma = kTRUE;
        if(fGammaElectronCuts && (!fGammaElectronCuts->IsSelected(legPos) ||
          !fGammaElectronCuts->IsSelected(legNeg)))
        { goodGamma = kFALSE; }
      }
    }

    Bool_t veryGoodK0s     = kFALSE;
    Bool_t veryGoodLambda  = kFALSE;
    Bool_t veryGoodALambda = kFALSE;
    Bool_t veryGoodGamma   = kFALSE;
    if(fV0StrongCuts && (goodK0s || goodLambda || goodALambda || goodGamma)) {
      ...
      if(processV0 && goodK0s && TMath::Abs(pdgV0)==310 && TMath::Abs(pdgP)==211 && TMath::Abs(pdgN)==211)
        veryGoodK0s = kTRUE;
      if(processV0 && goodLambda && pdgV0==3122 && (TMath::Abs(pdgP)==211 || TMath::Abs(pdgP)==2212) &&
        (TMath::Abs(pdgN)==211 || TMath::Abs(pdgN)==2212))
        veryGoodLambda = kTRUE;
      if(processV0 && goodALambda && pdgV0==-3122 && (TMath::Abs(pdgP)==211 || TMath::Abs(pdgP)==2212) &&
        (TMath::Abs(pdgN)==211 || TMath::Abs(pdgN)==2212))
    ...
    if(!((goodK0s     && fFillK0s)     || (goodLambda  && fFillLambda)  ||
         (goodALambda && fFillALambda) || (goodGamma   && fFillGammaConversions)))
    { continue; }

    // Fill the V0 information into the tree for 4 hypothesis: K0s, Lambda, Anti-Lambda and gamma conversion
    AliReducedPairInfo* k0sReducedPair     = FillV0PairInfo(v0, AliReducedPairInfo::kK0sToPiPi,     legPos,
                                                            legNeg, &primaryVertexKF, v0ChargesAreCorrect);
    AliReducedPairInfo* lambdaReducedPair  = FillV0PairInfo(v0, AliReducedPairInfo::kLambda0ToPPi,  legPos,
                                                            legNeg, &primaryVertexKF, v0ChargesAreCorrect);
    AliReducedPairInfo* alambdaReducedPair = FillV0PairInfo(v0, AliReducedPairInfo::kALambda0ToPPi, legPos,
                                                            legNeg, &primaryVertexKF, v0ChargesAreCorrect);
    AliReducedPairInfo* gammaReducedPair   = FillV0PairInfo(v0, AliReducedPairInfo::kGammaConv,     legPos,
                                                            legNeg, &primaryVertexKF, v0ChargesAreCorrect);

    if(fFillK0s && goodK0s && k0sReducedPair->fMass[0]>fK0sMassRange[0] &&
      k0sReducedPair->fMass[0]<fK0sMassRange[1])
    {
    ...
    if(fFillLambda && goodLambda && lambdaReducedPair->fMass[0]>fLambdaMassRange[0] &&
      lambdaReducedPair->fMass[0]<fLambdaMassRange[1])
    {
      TClonesArray& tracks = *(fReducedEvent->fCandidates);
    ...
    if(fFillALambda && goodALambda && alambdaReducedPair->fMass[0]>fLambdaMassRange[0] &&
       alambdaReducedPair->fMass[0]<fLambdaMassRange[1])
    {
      TClonesArray& tracks = *(fReducedEvent->fCandidates);
    ...
    if(fFillGammaConversions && goodGamma && gammaReducedPair->fMass[0]>fGammaMassRange[0] &&
      gammaReducedPair->fMass[0]<fGammaMassRange[1])
    {
      TClonesArray& tracks = *(fReducedEvent->fCandidates);
  ...
  AliESDtrack* legPos, AliESDtrack* legNeg, AliKFVertex* vtxKF, Bool_t chargesAreCorrect)
{
  ...
  if(!reducedPair->fPairType) {  // offline
    ...
    if(reducedPair->fP[1] < 0.0)
      reducedPair->fP[1] = 2.0*TMath::Pi() + reducedPair->fP[1];  // converted to [0,2pi]
    reducedPair->fP[0]          = v0->Pt();
    reducedPair->fP[2]          = v0->Eta();
    reducedPair->fLxy           = v0->GetRr();
    reducedPair->fPointingAngle = v0->GetV0CosineOfPointingAngle(vtxKF->GetX(), vtxKF->GetY(),
                                                                 vtxKF->GetZ());
    ...
    Double_t deltaPos[3];
    deltaPos[0] = v0Refit.GetX() - vtxKF->GetX();
    deltaPos[1] = v0Refit.GetY() - vtxKF->GetY();
    deltaPos[2] = v0Refit.GetZ() - vtxKF->GetZ();
    Double_t momV02    = v0Refit.GetPx()*v0Refit.GetPx() + v0Refit.GetPy()*v0Refit.GetPy() +
                         v0Refit.GetPz()*v0Refit.GetPz();
    Double_t deltaPos2 = deltaPos[0]*deltaPos[0] + deltaPos[1]*deltaPos[1] + deltaPos[2]*deltaPos[2];
    reducedPair->fPointingAngle = (deltaPos[0]*v0Refit.GetPx() + deltaPos[1]*v0Refit.GetPy() +
                                   deltaPos[2]*v0Refit.GetPz()) / TMath::Sqrt(momV02*deltaPos2);
...
void AliAnalysisTaskReducedTreeMaker::FillV0PairInfoAOD()
{
  ...
  for(Int_t iV0=0; iV0<InputEvent()->GetNumberOfV0s(); ++iV0) {  // loop over V0s
    ...
    if(fV0CutsAOD) {
      ...
      if(processV0 && TMath::Abs(pdgV0)==22 && TMath::Abs(pdgP)==11 && TMath::Abs(pdgN)==11) {
        goodGamma = kTRUE;
        if(fGammaElectronCuts &&
          (!fGammaElectronCuts->IsSelected(legPos) || !fGammaElectronCuts->IsSelected(legNeg)))
        { goodGamma = kFALSE; }
      }
    }
    ...
    // Fill the V0 information into the tree for 4 hypothesis: K0s, Lambda, Anti-Lambda and gamma conversion
    AliReducedPairInfo* k0sReducedPair     = FillV0PairInfoAOD(v0, AliReducedPairInfo::kK0sToPiPi,     
                                                               legPos, legNeg, &primaryVertexKF);
    AliReducedPairInfo* lambdaReducedPair  = FillV0PairInfoAOD(v0, AliReducedPairInfo::kLambda0ToPPi,  
                                                               legPos, legNeg, &primaryVertexKF);
    AliReducedPairInfo* alambdaReducedPair = FillV0PairInfoAOD(v0, AliReducedPairInfo::kALambda0ToPPi, 
                                                               legPos, legNeg, &primaryVertexKF);
    AliReducedPairInfo* gammaReducedPair   = FillV0PairInfoAOD(v0, AliReducedPairInfo::kGammaConv,     
                                                               legPos, legNeg, &primaryVertexKF);
...
AliReducedPairInfo* AliAnalysisTaskReducedTreeMaker::FillV0PairInfoAOD(AliAODv0* v0, Int_t id,
  AliAODTrack* legPos, AliAODTrack* legNeg, AliKFVertex* vtxKF)
{
  //
  // Create a reduced V0 object and fill it
  //
...
UChar_t AliAnalysisTaskReducedTreeMaker::EncodeTPCClusterMap(AliVParticle* track, Bool_t isAOD)
{
  ...
  else      esdTrack = static_cast<AliESDtrack*>(track);
...
//_________________________________________________________________________________
Int_t AliAnalysisTaskReducedTreeMaker::GetSPDTrackletMultiplicity(AliVEvent* event, Float_t lowEta,
                                                                  Float_t highEta)
{
  //
