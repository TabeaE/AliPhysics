...
class AliReducedPairInfo : public AliReducedBaseTrack {
  ...
  void SetNTracksRegions (Int_t n, Int_t iRegion, Int_t icut=0) {
    if(n>=0 && iRegion>=0 && iRegion<=2) fNTracksRegions[8*iRegion+icut] = n;}
 ...
 protected:
  ...
  Char_t   fPairType;       // 0- offline, 1- on the fly for V0 candidates; 0 ++; 1 +-;
                            // 2 -- for other pairs;
...
inline Float_t AliReducedPairInfo::Energy() const 
{
  ...
  switch (fCandidateId) {
    case kK0sToPiPi:     mass = fMass[0]; break;
    case kLambda0ToPPi:  mass = fMass[1]; break;
    case kALambda0ToPPi: mass = fMass[2]; break;
    case kGammaConv:     mass = fMass[3]; break;
    default:             mass = fMass[0]; break;    
  }
  Float_t p = P();
...
inline Float_t AliReducedPairInfo::Rapidity() const
{
  ...
  Float_t e = Energy();
  Float_t pz = Pz();
  if(e-TMath::Abs(pz) > 1.0e-10) return 0.5 * TMath::Log((e+pz)/(e-pz));
  else                           return -999.;
}
