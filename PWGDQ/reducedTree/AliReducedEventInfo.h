...
class AliReducedEventInfo : public AliReducedBaseEvent {
 ...
 public:
  ...
  Bool_t    L0TriggerInput(UShort_t bit)  const {return (bit<8*sizeof(UInt_t) ?
                                                        (fL0TriggerInputs&(UInt_t(1)<<bit)) : kFALSE);}
  UInt_t    L1TriggerInputs()             const {return fL1TriggerInputs;}
  Bool_t    L1TriggerInput(UShort_t bit)  const {return (bit<8*sizeof(UInt_t) ?
                                                        (fL1TriggerInputs&(UInt_t(1)<<bit)) : kFALSE);}
  UShort_t  L2TriggerInputs()             const {return fL2TriggerInputs;}
  Bool_t    L2TriggerInput(UShort_t bit)  const {return (bit<8*sizeof(UShort_t) ?
                                                        (fL2TriggerInputs&(UShort_t(1)<<bit)) : kFALSE);}
  ...
  Float_t   TPCpileupZ(Int_t side=0)             const {return (side<0||side>2 ? -999. :
  ...
  Short_t   SPDFiredChips(Int_t layer)        const {return (layer==1 || layer==2 ? fSPDFiredChips[layer-1]
                                                                                  : -999);}
  ...
  Double_t GetEventPlaneStatus(Int_t detector, Int_t harmonic) const {
    return fEventPlane.GetEventPlaneStatus(detector, harmonic);}
 ...
 protected:
  ...
  Float_t   fMultiplicityEstimators[13];  // multiplicity estimators: "OnlineV0M", "OnlineV0A", "OnlineV0C",
                                          // "ADM", "ADA", "ADC", "SPDClusters", "SPDTracklets", "RefMult05",
                                          // "RefMult08"
  Float_t   fMultiplicityEstimatorPercentiles[13];  // multiplicity estimators: "OnlineV0M", "OnlineV0A",
                                                    // "OnlineV0C", "ADM", "ADA", "ADC", "SPDClusters",
                                                    // "SPDTracklets", "RefMult05", "RefMult08"
  ...
  Float_t   fTPCpileupZ2[2];             // TPC pileup event Z position computed with larger DCA cut;
                                         // [0]: A-side; [1]: C-side
  Int_t     fTPCpileupContributors2[2];  // TPC pileup event contributors computed with larger DCA cut;
                                         // [0]: A-side; [1]: C-side
  ...
  Int_t     fSPDnSingle;            // number of clusters in SPD layer 1,
                                    // not associated to a tracklet on SPD layer 2
  ...
  Int_t     fNch[8];                      // number of MCtruth charged particles in different eta regions
                                          // (for MC with pileup this includes pileup tracks)
  ...
  ClassDef(AliReducedEventInfo, 16);
};
