...
class AliReducedTrackInfo : public AliReducedBaseTrack {
 ...
 public:
  ...
  Bool_t   ITSClusterIsShared(Int_t layer) const {
              return (layer>=0 && layer<6 ? (fITSSharedClusterMap&(1<<layer)) : kFALSE);};
  ...
  Float_t PMC      () const {
    return (fIsMCTruth ? P() : TMath::Sqrt(fMCMom[0]*fMCMom[0]+fMCMom[1]*fMCMom[1]+fMCMom[2]*fMCMom[2]));}
  Float_t PhiMC      ()                       const;
  Float_t EtaMC      ()                       const;
  Float_t RapidityMC (Float_t massAssumption) const;
  Float_t ThetaMC    ()                       const;
  Float_t EnergyMC   (Float_t massAssumption) const {
    return TMath::Sqrt(massAssumption*massAssumption+PMC()*PMC());}
 ...
 protected:
  ...
  // TRD
  UChar_t fTRDntracklets[2];    // 0 - AliESDtrack::GetTRDntracklets();
                                // 1 - AliESDtrack::GetTRDntrackletsPID()   TODO: use only 1 char
...
inline Float_t AliReducedTrackInfo::ThetaMC() const {
   ...
   Float_t p = PMC();
   if(p >= 1.0e-6) return TMath::ACos(fMCMom[2]/p);
   else            return 0.0;
}
