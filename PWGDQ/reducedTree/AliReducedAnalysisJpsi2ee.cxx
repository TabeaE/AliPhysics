...
void AliReducedAnalysisJpsi2ee::AddTrackCut(AliReducedInfoCut* cut) {
    ...
    if (fPairCuts.GetEntries()>1) {
        ...
        for (Int_t iPairCut=0; iPairCut<fPairCuts.GetEntries(); iPairCut++) {
            ...
            histClassNames += Form("PairMEPP_%s_%s;",
                                   fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName());
            histClassNames += Form("PairMEPM_%s_%s;",
                                   fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName());
            histClassNames += Form("PairMEMM_%s_%s;",
                                   fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName());
            }
        }
    } else {
...
void AliReducedAnalysisJpsi2ee::AddPairCut(AliReducedInfoCut* cut) {
    ...
    if(fPairCuts.GetEntries()>1) {
        ...
        for (Int_t iPairCut=0; iPairCut<fPairCuts.GetEntries(); iPairCut++) {
            for (Int_t iTrackCut=0; iTrackCut<fTrackCuts.GetEntries(); iTrackCut++) {
                histClassNamesNew += Form(
                    "PairMEPP_%s_%s;", fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName());
                histClassNamesNew += Form(
                    "PairMEPM_%s_%s;", fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName());
                histClassNamesNew += Form(
                    "PairMEMM_%s_%s;",  fTrackCuts.At(iTrackCut)->GetName(), fPairCuts.At(iPairCut)->GetName());
            }
        }
...
void AliReducedAnalysisJpsi2ee::Init() {
    ...
    fHistosManager->SetDefaultVarNames(
        AliReducedVarManager::fgVariableNames,AliReducedVarManager::fgVariableUnits);

    fMixingHandler->SetHistogramManager(fHistosManager);

    if (fClusterTrackMatcher) {
        fClusterTrackMatcherMultipleMatchesBefore = new TH1I(
            "multipleCounts_beforeMatching","mulitple counts of matched cluster IDs beofore matching", 50, 0.5,
            50.5);
        fClusterTrackMatcherMultipleMatchesAfter = new TH1I(
            "multipleCounts_afterMatching", "mulitple counts of matched cluster IDs after matching", 50, 0.5,
            50.5);
...
void AliReducedAnalysisJpsi2ee::Process() {
    ...
    if(fOptionLoopOverTracks) {
        ...
        fValues[AliReducedVarManager::kNtracksAnalyzed] = fValues[AliReducedVarManager::kNtracksNegAnalyzed]
                                                        + fValues[AliReducedVarManager::kNtracksPosAnalyzed];
        fValues[AliReducedVarManager::kEvAverageTPCchi2] /= (fPosTracks.GetEntries()+fNegTracks.GetEntries()>0 ? 
            fValues[AliReducedVarManager::kNtracksAnalyzed] : 1.0); 
    }
...
void AliReducedAnalysisJpsi2ee::FillTrackHistograms(TString trackClass /*= "Track"*/) {
    ...
    for(Int_t i=0;i<fPosTracks.GetEntries();++i) {
        ...
        fValues[AliReducedVarManager::kNtracksAnalyzedInPhiBins+(track->Eta()<0.0 ? 0 : 18) +
            TMath::FloorNint(18.*track->Phi()/TMath::TwoPi())] += 1;
    ...
    if (fClusterTrackMatcher) {
        fClusterTrackMatcher->FillMultipleMatchesHistogram(fClusterTrackMatcherMultipleMatchesBefore,
                                                           fClusterTrackMatcher->GetMatchedClusterIDsBefore());
        fClusterTrackMatcher->FillMultipleMatchesHistogram(fClusterTrackMatcherMultipleMatchesAfter,
                                                           fClusterTrackMatcher->GetMatchedClusterIDsAfter());
        fClusterTrackMatcher->ClearMatchedClusterIDsBefore();
        fClusterTrackMatcher->ClearMatchedClusterIDsAfter();
    }
...
void AliReducedAnalysisJpsi2ee::FillTrackHistograms(AliReducedBaseTrack* track, TString trackClass /*="Track"*/) {
    ...
    for(Int_t icut=0; icut<fTrackCuts.GetEntries(); ++icut) {
        if(track->TestFlag(icut)) {
            fHistosManager->FillHistClass(Form("%s_%s", trackClass.Data(), fTrackCuts.At(icut)->GetName()),
                                          fValues);
                    ...
                    for(Int_t iMC=0; iMC<=fLegCandidatesMCcuts.GetEntries(); ++iMC) {
                        if(mcDecisionMap & (UInt_t(1)<<iMC))
                            fHistosManager->FillHistClass(Form("%sStatusFlags_%s_%s", trackClass.Data(),
                                                               fTrackCuts.At(icut)->GetName(),
                                                               fLegCandidatesMCcuts.At(iMC)->GetName()),
                                                          fValues);
                    }
                }
            }
            ...
            for(Int_t iLayer=0; iLayer<6; ++iLayer) {
                ...
                fHistosManager->FillHistClass(Form("%sITSsharedClusterMap_%s",
                                                   trackClass.Data(), fTrackCuts.At(icut)->GetName()), fValues);
                if(mcDecisionMap) {
                    for(Int_t iMC=0; iMC<=fLegCandidatesMCcuts.GetEntries(); ++iMC) {
                        if(mcDecisionMap & (UInt_t(1)<<iMC))
            ...
            for(Int_t iLayer=0; iLayer<8; ++iLayer) {
                ...
                fHistosManager->FillHistClass(Form("%sTPCclusterMap_%s", trackClass.Data(),
                                                   fTrackCuts.At(icut)->GetName()), fValues);
                if(mcDecisionMap) {
                    for(Int_t iMC=0; iMC<=fLegCandidatesMCcuts.GetEntries(); ++iMC) {
                        if(mcDecisionMap & (UInt_t(1)<<iMC))
                            fHistosManager->FillHistClass(Form("%sTPCclusterMap_%s_%s", trackClass.Data(),
                                                               fTrackCuts.At(icut)->GetName(),
                                                               fLegCandidatesMCcuts.At(iMC)->GetName()),
                                                          fValues);
                    }
...
//___________________________________________________________________________
void AliReducedAnalysisJpsi2ee::FillPairHistograms(ULong_t trackMask, ULong_t pairMask, Int_t pairType,
                                                   TString pairClass /*="PairSE"*/, UInt_t mcDecisions /* = 0*/)
{
   //
...
//___________________________________________________________________________
void AliReducedAnalysisJpsi2ee::FillClusterHistograms(AliReducedCaloClusterInfo* cluster,
                                                      TString clusterClass/*="CaloCluster"*/) {
  //
  // fill cluster histograms
  //
...
void AliReducedAnalysisJpsi2ee::LoopOverTracks1() {
    ...
    for (Int_t it=0; it<trackList->GetEntries(); ++it) {
        ...
        // reset track variables
        for (Int_t i=AliReducedVarManager::kNEventVars; i<AliReducedVarManager::kEMCALmatchedEOverP; ++i)
            fValues[i]=-9999.;
...
void AliReducedAnalysisJpsi2ee::LoopOverTracks2() {
    //
    fTrackSelected.Clear("C");
    fValues[AliReducedVarManager::kNtracks2Selected] = 0; 
    ...
    // For multiplicity in regions of azimuthal angle 
    ...
    for(Int_t jd=0; jd<trackList->GetEntries(); ++jd) {
        ...
        // track->TestQualityFlag(38) not neccessary for electrons; TODO check
            GoodTracks ++;
    ...
    fValues[AliReducedVarManager::kNtracks2Selected]= GoodTracks; 
}
    ...
    for(Int_t ip=0; ip<fPosTracks.GetEntries(); ++ip) {
        ...
        for(Int_t in=0; in<fNegTracks.GetEntries(); ++in) {
            ...
            if(pairCutMask) {
                ...
                TIter nextJpsiTrack(&fTrackSelected);
                Double_t jpsi_phi = fValues[AliReducedVarManager::kPhi];
                ...
                while(AliReducedBaseTrack* JpsiTrack = (AliReducedBaseTrack*)nextJpsiTrack()) {
                    ...
                    if ((5.*TMath::Pi()/3. < delta_Jpsiphi  &&  delta_Jpsiphi < 2.*TMath::Pi()) ||
                        (delta_Jpsiphi < TMath::Pi()/3.  &&  delta_Jpsiphi > 0.*TMath::Pi()))
                    {
                        nJpsi_toward ++;
                    }
                ...
                // FillPairHistograms: 1 is for +- pairs
                FillPairHistograms(pTrack->GetFlags() & nTrack->GetFlags(), pairCutMask, 1, pairClass,
                                   (fOptionRunOverMC ? CheckReconstructedLegMCTruth(pTrack, nTrack) : 0));
                fValues[AliReducedVarManager::kNpairsSelected] += 1.0;  // TODO before or after store candidates?
                if(fOptionStoreJpsiCandidates) {
                    ...
                    pair->PtPhiEta(fValues[AliReducedVarManager::kPt], fValues[AliReducedVarManager::kPhi],
                                   fValues[AliReducedVarManager::kEta]);
        ...
        if(fOptionRunLikeSignPairing) {
            for(Int_t ip2=ip+1; ip2<fPosTracks.GetEntries(); ++ip2) {
                ...
                ULong_t pairCutMask = IsPairSelected(fValues);
                if(pairCutMask) {
                    // FillPairHistograms: 0 is for ++ pairs
                    FillPairHistograms(pTrack->GetFlags() & pTrack2->GetFlags(), pairCutMask, 0, pairClass);       
                    fValues[AliReducedVarManager::kNpairsSelected] += 1.0;
                    if(fOptionStoreJpsiCandidates) {
                        AliReducedPairInfo* pair = new AliReducedPairInfo();
                        pair->SetFlags(pTrack->GetFlags() & pTrack2->GetFlags());
                        pair->SetQualityFlags(pairCutMask);
                        pair->PtPhiEta(fValues[AliReducedVarManager::kPt],
                                       fValues[AliReducedVarManager::kPhi],
                                       fValues[AliReducedVarManager::kEta]);
                        pair->SetMass(fValues[AliReducedVarManager::kMass]);
                        pair->CandidateId(AliReducedPairInfo::kJpsiToEE);
                        pair->PairType(0);
    ...
    if(fOptionRunLikeSignPairing) {
        ...
        for(Int_t in=0; in<fNegTracks.GetEntries(); ++in) {
            ...
            for(Int_t in2=in+1; in2<fNegTracks.GetEntries(); ++in2) {
                ...
                ULong_t pairCutMask = IsPairSelected(fValues);
                if(pairCutMask) {
                    // FillPairHistograms: 2 is for -- pairs
                    FillPairHistograms(nTrack->GetFlags() & nTrack2->GetFlags(), pairCutMask, 2, pairClass);      
                    fValues[AliReducedVarManager::kNpairsSelected] += 1.0;
                    if(fOptionStoreJpsiCandidates) {
                        AliReducedPairInfo* pair = new AliReducedPairInfo();
...
//___________________________________________________________________________
UInt_t AliReducedAnalysisJpsi2ee::CheckReconstructedLegMCTruth(AliReducedBaseTrack* ptrack,
...
void AliReducedAnalysisJpsi2ee::FillMCTruthHistograms() {
  ...
  //       full track, so there is no point in looping over the second track array which, if it exists, contains 
  //       just base tracks
  //LoopOverMCTracks(2);
}
...
void AliReducedAnalysisJpsi2ee::LoopOverMCTracks(Int_t trackArray /*=1*/) {
   ...
   // Loop through the list of pure MC particles and find the required mothers and their corresponding daughters
    ...
    for(Int_t it=0; it<trackList->GetEntries(); ++it) {
        ...
        TIter nextnoJpsiTrack(&fTrackSelected);
        Double_t motherphi = mother->PhiMC();
        ...
        while(AliReducedBaseTrack* noJpsiTrack = (AliReducedBaseTrack*)nextnoJpsiTrack()) {
            ...
            if ((5.*TMath::Pi()/3. < delta_JpsiphiMC && delta_JpsiphiMC < 2.*TMath::Pi()) ||
                (delta_JpsiphiMC < TMath::Pi()/3.   && delta_JpsiphiMC > 0.*TMath::Pi())) nJpsi_towardMC ++;
            
            if((delta_JpsiphiMC > TMath::Pi()/3.   && delta_JpsiphiMC < 2.*TMath::Pi()/3.) ||
               (delta_JpsiphiMC > 4.*TMath::Pi()/3. && delta_JpsiphiMC < 5.*TMath::Pi()/3.))
                nJpsi_transverseMC ++;
            
            if(delta_JpsiphiMC > 2.*TMath::Pi()/3. && delta_JpsiphiMC < 4.*TMath::Pi()/3.) nJpsi_awayMC ++; 
    ...
    // Loop through the list of pure MC particles and find those signals which do not require a mother (e.g. 
    // pairs of electrons without a mother)
...
//___________________________________________________________________________
void AliReducedAnalysisJpsi2ee::FindJpsiTruthLegs(AliReducedTrackInfo* mother, Int_t& leg1Label,
                                                  Int_t& leg2Label) {
   //
