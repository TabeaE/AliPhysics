...
public:
  ...
  void ConfigAnalysisUtils (Int_t minVtxContrib=1, Double_t maxVtxZ=10.0, Bool_t cutOnSPDVtxZ=kTRUE) {
    fMinVtxContributors = minVtxContrib;
    fMaxVtxZ            = maxVtxZ;
    fCutOnSPDVtxZ       = cutOnSPDVtxZ;
  }
  ...
  void AddTrackFilter (AliAnalysisCuts* const filter, Bool_t option=kFALSE, Int_t minSel=-1,
                       Int_t maxSel=-1);
  ...
  void SetK0sMassRange      (Double_t min=0.4,  Double_t max=0.6)  {fK0sMassRange[0]    = min;
                                                                    fK0sMassRange[1]    = max;}
  void SetLambdaMassRange   (Double_t min=1.08, Double_t max=1.15) {fLambdaMassRange[0] = min;
                                                                    fLambdaMassRange[1] = max;}
  void SetGammaConvMassRange(Double_t min=0.0,  Double_t max=0.1)  {fGammaMassRange[0]  = min;
                                                                    fGammaMassRange[1]  = max;}
  ...
  void SetFillV0Info           (Bool_t flag=kTRUE, Bool_t writeEventsWithOnlyV0s=kFALSE) {
    fFillV0Info             = flag;
    fWriteEventsWithOnlyV0s = writeEventsWithOnlyV0s;
  }
  ...
  void SetFillEventPlaneInfo   (Bool_t flag=kTRUE, Float_t tpcEtaGap=1.0) {
    fFillEventPlaneInfo  = flag;
    fEventPlaneTPCetaGap = tpcEtaGap;}
  ...
  void SetFillTRDMatchedTracks (Bool_t flag1=kTRUE, Bool_t flag2=kFALSE) {
    fFillTRDMatchedTracks    = flag1;
    fFillAllTRDMatchedTracks = flag2;
  }
 ...
 private:
  ...
  Bool_t   fWriteTree;                    // If kFALSE don't write the tree, use task only to produce
                                          //  on the fly reduced events.
  Double_t fScaleDownEvents;              // Allow writing events which do not fulfill the minimum nof 
                                          //  tracks criteria with scale down factor (default: 0.).
  Bool_t   fWriteSecondTrackArray;        // Write second array only if full+base tracks requested
  std::vector<Bool_t> fWriteBaseTrack;    // Specifier if tracks for certain track filter are reduced or
                                          //  base tracks.
  std::vector<Int_t>  fMinSelectedTracks;    // Array of min required sel. tracks for each track filter
  std::vector<Int_t>  fMaxSelectedTracks;    // Array of max required sel. tracks for each track filter
  std::vector<Int_t>  fNSelectedFullTracks;  // Array of nof full (ReducedTrack) selected tracks in the
                                             // Current event for each track filter + for V0 prongs
  std::vector<Int_t>  fNSelectedBaseTracks;  // Array of number of selected base tracks in the current 
                                             //  event for each track filter + for V0 prongs
  ...
  Bool_t  fWriteEventsWithOnlyV0s;  // (default: false) if true, an event which contains just V0 legs 
                                    // will be written, even if no track selection criteria is met
  ...
  TList   fMCsignals;               // list of AliSignalMC objects to select which particles from the 
                                    // Kinematics stack will be written in the tree
  Int_t   fMCsignalsWritingOptions[kMaxMCsignals];  // Writing options for each of the MC signals
                                                    // (either base or full track)
  Bool_t  fFillTRDMatchedTracks;     // Write global tracks with matched TRD tracks
  Bool_t  fFillAllTRDMatchedTracks;  // If true, fill all global tracks matched in TRD; if false, fill 
                                     //  just those global tracks which were selected with the track of 
                                     //  V0 filters.
  ...
  AliReducedPairInfo* FillV0PairInfo    (AliESDv0* v0, Int_t id, AliESDtrack* legPos,
                                         AliESDtrack* legNeg, AliKFVertex* vtxKF,
                                         Bool_t chargesAreCorrect);
  void                FillV0PairInfoAOD ();  // fill V0 reduced pair information
  AliReducedPairInfo* FillV0PairInfoAOD (AliAODv0* v0, Int_t id, AliAODTrack* legPos,
                                         AliAODTrack* legNeg, AliKFVertex* vtxKF);
  ...
  UInt_t MatchMCsignals (Int_t iparticle, Bool_t isAOD);  // Check all MC signals for MC particle i and 
                                                          //  write decisions in a bit map.
  Bool_t CheckMCtruthWriteFormat (UInt_t bitMap);         // Make a decision on which format (base/full) 
                                                          //  to be used for the MC track
  ...
  void   FillStatisticsHistograms (Bool_t isSelected, UInt_t physSel, UChar_t trdTrigMap,
                                   UInt_t emcalTrigMap, Double_t xbin, Double_t* percentiles,
                                   Int_t nEstimators);
  void   FillTrackStatisticsHistogram ();
  
  Bool_t IsTrackSelected                   (AliVParticle* track, Double_t* values,
  ...
  void   SetTrackFilterQualityFlags        (AliReducedBaseTrack* track,
                                            std::vector<Bool_t> filterDecision);
  ...
  ClassDef(AliAnalysisTaskReducedTreeMaker, 20);  // Analysis Task for creating a reduced event information tree
};
#endif
