...
public:
  ...
  void SetFillV0Info           (Bool_t flag=kTRUE, Bool_t writeEventsWithOnlyV0s=kFALSE) {fFillV0Info = flag;
  ...
  void SetFillEventPlaneInfo   (Bool_t flag=kTRUE, Float_t tpcEtaGap=1.0) {fFillEventPlaneInfo = flag;
  ...
  void SetFillTRDMatchedTracks (Bool_t flag1=kTRUE, Bool_t flag2=kFALSE) {fFillTRDMatchedTracks = flag1;
 ...
 private:
  ...
  Double_t fScaleDownEvents;              // allow writing events which do not fulfill the minimum number of
                                          // tracks criteria with scale down factor (default is zero)
  ...
  std::vector<Int_t>  fNSelectedFullTracks;  // array of number of full (ReducedTrack) selected tracks in the
                                             // current event for each track filter + for V0 prongs
  std::vector<Int_t>  fNSelectedBaseTracks;  // array of number of selected base tracks in the current event
                                             // for each track filter + for V0 prongs
  ...
  TH1F*  fMCPlpEventsHistogram;
  TH1D*  fMCPlpParticlesHistogram;
  ...
  Bool_t  fWriteEventsWithOnlyV0s;  // (default: false) if true, an event which contains just V0 legs will be
                                    // written, even if no track selection criteria is met
  ...
  TList   fMCsignals;               // list of AliSignalMC objects to select which particles from the Kinematics
                                    // stack will be written in the tree
  ...
  Bool_t  fFillAllTRDMatchedTracks;  // if true, fill all global tracks matched in TRD; if false, fill just those
                                     // global tracks which were selected with the track of V0 filters
  ...
  AliReducedPairInfo* FillV0PairInfo(AliESDv0* v0, Int_t id, AliESDtrack* legPos, AliESDtrack* legNeg,
                                     AliKFVertex* vtxKF, Bool_t chargesAreCorrect);
  void FillV0PairInfoAOD();            // fill V0 reduced pair information
  AliReducedPairInfo* FillV0PairInfoAOD(AliAODv0* v0, Int_t id, AliAODTrack* legPos, AliAODTrack* legNeg,
                                        AliKFVertex* vtxKF);
  ...
  UInt_t MatchMCsignals(Int_t iparticle, Bool_t isAOD);  // check all MC signals for MC particle i and write
                                                         // decisions in a bit map
  Bool_t CheckMCtruthWriteFormat(UInt_t bitMap);         // make a decision on which format (base/full) to be
                                                         // used for the MC track
  ...
  void   FillStatisticsHistograms(Bool_t isSelected, UInt_t physSel, UChar_t trdTrigMap, UInt_t emcalTrigMap,
                                  Double_t xbin, Double_t* percentiles, Int_t nEstimators);
