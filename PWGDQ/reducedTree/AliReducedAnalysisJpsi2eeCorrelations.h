...
public:
  ...
  void AddJpsiElectronCut    (AliReducedInfoCut* cut) {AddTrackCut(cut);}  // synonym function to AddTrackCut
  ...
  void SetLoopOverTracks   (Bool_t option) {
    ...
    if(!fOptionLoopOverTracks) {
      fOptionRunPairing       = kFALSE;
      fOptionRunMixing        = kFALSE;
      fOptionUseLikeSignPairs = kFALSE;
      fOptionRunCorrelation   = kFALSE;
    }
  }
  ...
  const Char_t*     GetAssociatedTrackCutName (Int_t i) const {
    return (i<fAssociatedTrackCuts.GetEntries() ? fAssociatedTrackCuts.At(i)->GetName() : "");}
...
protected:
  ...
  Bool_t fOptionRunCorrelation;       // true:  run correlation of candidates and associated tracks, 
                                      // false: apply only associated track cuts
  ...
  TList fAssociatedTracksMB;    // list of selected associated tracks in MB events
                                //   (for mixing with EMCal trigger)
  ...
  void LoopOverAssociatedTracks      (Int_t arrayOption=1, Bool_t fillHistograms=kTRUE,
                                      Bool_t fillMBTracks=kFALSE);
  ...
  void FillCorrelationHistograms     (AliReducedPairInfo* jpsi, AliReducedBaseTrack* assoc,
                                      TString corrClass="CorrSE", Bool_t isMCTruth=kFALSE);
