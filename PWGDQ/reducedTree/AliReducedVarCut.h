...
class AliReducedVarCut : public AliReducedInfoCut {
 ...
 public:
  ...
  // NOTE: If a dependent variable is specified, then the selection is applied only if the dependent variable 
  //       is in the range [depCutLow,depCutHigh] or outside if "depCutExclude" is true
  void AddCut(Int_t var, Float_t cutLow, Float_t cutHigh, Bool_t exclude=kFALSE, 
              ...
              Bool_t depCutExclude=kFALSE, Int_t dependentVar2=AliReducedVarManager::kNothing,
              Float_t depCut2Low=0., Float_t depCut2High=0., Bool_t depCut2Exclude=kFALSE);
  // NOTE: Define cuts which use functions of a defined variable instead of a constant cut; the logic of the 
  //       arguments is the same as for the above function
